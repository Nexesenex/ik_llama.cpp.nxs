...
void print_usage(const char* prog) {
    std::cerr << "Usage: " << prog << " --devices 0,1 [--interval 10] [--temp-limit 85]\n"
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " --devices 0,1 [--interval 10] [--temp-limit 85]\n";
        return 1;
    }
    ...
    while (running) {
        ...
        if (!to_poll.empty()) {
            light_heartbeat(to_poll);
        }
        iter++;
        if (iter % temp_check_iters == 0) {
            for (int d : active_devices) {
