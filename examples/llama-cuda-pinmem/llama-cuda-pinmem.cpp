...
static void print_usage(const char * prog) {
    ...
    LOG_INFO("  -v, --verbose    Enable verbose logging per iteration\n");
    ...
    LOG_INFO("  -d, --dev N      Test a specific device only (repeatable)\n");
    LOG_INFO("\n");
...
int main(int argc, char ** argv) {
    ...
    for (size_t idx = 0; idx < to_test.size(); idx++) {
        ...
        char vram_str[24];
        if (dev.total_vram >= (1024LL * 1024LL * 1024LL)) {
            snprintf(vram_str, sizeof(vram_str), "%.2f GiB", dev.total_vram / (1024.0 * 1024.0 * 1024.0));
        } else {
            snprintf(vram_str, sizeof(vram_str), "%.0f MiB", dev.total_vram / (1024.0 * 1024.0));
        }
