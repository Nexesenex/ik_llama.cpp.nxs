...
// Canonical definition lives in server-context-chat.cpp (also used standalone
// by the test-server-chat target). It was duplicated here by PR 2476 and only
// diverged by the is_partial fix, so this copy is removed to fix LNK2005.
...
bool server_context::l_s_w_t(server_slot& slot, server_task& task) {
    ...
    slot.sparams.break_endless_sentences = json_value(data, "break_endless_sentences", default_sparams.break_endless_sentences);
...
json server_context::get_formatted_generation(const server_slot& slot) const {
    ...
    return json{
        ...
        {"break_endless_sentences",   slot.sparams.break_endless_sentences},
            ...
            if (!is_active_slot || slot.i_batch < (int)i || slot.i_batch >= (int)(i + n_tokens)) {
                ...
                if (slot.command == SLOT_COMMAND_LOAD_PROMPT) {
                    if (slot.do_checkpoint) {
                        if (params_base.ctx_checkpoints_interval_gating) {
                            create_checkpoint_at_interval(slot), true;
                        } else {
