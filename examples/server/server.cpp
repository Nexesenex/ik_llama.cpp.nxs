...
struct server_response {
    ...
    void add_waiting_task_id(int id_task) {
        ...
        // LOG_VERBOSE("add task %d to waiting list. current waiting = %d (before add)\n", id_task, (int) waiting_task_ids.size());
    ...
    void remove_waiting_task_id(int id_task) {
        ...
        // LOG_VERBOSE("remove task %d from waiting list. current waiting = %d (before remove)\n", id_task, (int) waiting_task_ids.size());
...
struct server_context {
    ...
    bool process_token(completion_token_output & result, server_slot & slot) {
        ...
        if (slot.n_decoded >= slot.n_ctx) {
            ...
            // LOG_VERBOSE(slot, "stopped due to running out of context capacity, n_decoded = %d, n_ctx = %d\n", slot.n_decoded, slot.n_ctx);
        }
        ...
        // LOG_VERBOSE(slot, "n_decoded = %d, n_remaining = %d, next token: %5d '%s'\n", slot.n_decoded, slot.n_remaining, result.tok, token_str.c_str());
    ...
    void send_rerank(const server_slot & slot, const llama_batch & batch) {
        ...
        for (int i = 0; i < batch.n_tokens; ++i) {
            ...
            if (embd == NULL) {
                // LOG_VERBOSE(slot, "failed to get embeddings, token = %d, seq_id = %d\n", batch.token[i], batch.seq_id[i][0]);
        ...
        // LOG_VERBOSE(slot, "sending rerank result, res = '%s'\n", res.data.dump().c_str());
    ...
    std::vector<server_task> create_tasks_cmpl(json data, server_task_cmpl_type cmpl_type) {
        ...
        if (prompt.is_string() || json_is_array_of_numbers(prompt)) {
            ...
            if (cmpl_type == SERVER_TASK_CMPL_TYPE_RERANK) {
                ...
                LOG_VERBOSE("creating rerank tasks, n_prompts = %d\n", (int) prompts.size() - 1);
                ...
                LOG_VERBOSE("creating multi-prompt tasks, n_prompts = %d\n", (int) prompts.size());
                ...
                if (slot.state == SLOT_STATE_PROCESSING_PROMPT) {
                    ...
                    if (prompt_tokens.empty()) {
                        ...
                        switch (slot.cmpl_type) {
                            ...
                            // require slot.prompt to be array of 2 strings
                            if (!slot.prompt.is_array() || slot.prompt.size() != 2) {
                                // LOG_VERBOSE(slot, "%s", "invalid prompt for rerank task\n");
                                slot.release();
                                send_error(slot, "invalid prompt for rerank task", ERROR_TYPE_INVALID_REQUEST);
                                continue;
                            }
                        ...
                        // if (slot.inf_type == SERVER_TASK_INF_TYPE_EMBEDDING || slot.in
