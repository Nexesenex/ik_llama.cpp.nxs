//
// unit_test_cuda.cpp - byte-for-byte Q8_0 CUDA quantizer verification
//
// Three producers of Q8_0 GGUF quantized bytes, compared on identical input:
//
...
//   3. REF : local copy of quantize_row_q8_0_ref (ggml/src/ggml-quants.c:943)
//            with the fp16 step done by __float2half_rn, no ggml internals
//
...
//
// Layout note: block_q8_0 tiles the flat row-major F32 buffer contiguously
// (n_per_row % 32 == 0), so rows need no bookkeeping and per-slice calls can be
// concatenated. test_slices reproduces do_quantize's ne[2] slicing exactly:
// the CUDA branch and the CPU branch both quantize one expert slice at a time
// into consecutive slots, and both must equal a single whole-tensor call.
//
...
// conversion uses __float2half_rn, which is what GGML_FP32_TO_FP16 maps to and
// what the CUDA kernel's __half_as_ushort(__float2half_rn(d)) stores.
...
// Crafted edge-case blocks, one pattern per block.
...
static void dump_block(const char * who, const block_q8_0 * b) {
    printf("    %s: d=0x%04x qs=", who, (unsigned)b->d);
    for (int j = 0; j < QK8_0; ++j) printf("%02x", (uint8_t)b->qs[j]);
    printf("\n");
}

// Report the first differing quant block (d + all qs) plus the diff count.
static int64_t compare_buffers(const char * tag, const uint8_t * a, const uint8_t * b, size_t n) {
    int64_t ndiff = 0;
    int64_t first_blk = -1;
    const int64_t blk_bytes = (int64_t)sizeof(block_q8_0);
    for (size_t i = 0; i < n; ++i) {
        if (a[i] != b[i]) {
            ++ndiff;
            if (first_blk < 0) first_blk = (int64_t)i / blk_bytes;
        }
    }
    if (first_blk >= 0) {
        const block_q8_0 * ra = (const block_q8_0 *)(a + first_blk*blk_bytes);
        const block_q8_0 * rb = (const block_q8_0 *)(b + first_blk*blk_bytes);
...
static void test_one(const char * tag, int64_t nrows, int64_t n_per_row,
        void (*fill)(float *, int64_t), int device) {
    if (n_per_row % QK8_0 != 0 || nrows <= 0) return;
    ...
    const size_t nb_cpu = ggml_quantize_chunk(GGML_TYPE_Q8_0, src.data(), out_cpu.data(),
            0, nrows, n_per_row, nullptr, nullptr);

    // REF: local vanilla copy
    ref_quantize_q8_0(out_ref.data(), src.data(), nrows, n_per_row);
    const size_t nb_ref = out_size;

    // GPU: real CUDA path (always runs on device 0 in this POC)
    cudaSetDevice(device);
    const size_t nb_gpu = ggml_cuda_quantize_q8_0(src.data(), out_gpu.data(), nrows, n_per_row);
    ...
    const int64_t d_gpu_cpu = compare_buffers(tag, out_cpu.data(), out_gpu.data(), out_size);
    const int64_t d_cpu_ref = compare_buffers(tag, out_ref.data(), out_cpu.data(), out_size);
...
// the same bytes (quantize_q8_0 is contiguous, so it must).
static void test_slices(int device, int64_t ne0, int64_t ne1, int64_t ne2) {
    char tag[96];
    ...
    const size_t row_size    = ggml_row_size(GGML_TYPE_Q8_0, ne0);
    ...
    for (int64_t i02 = 0; i02 < ne2; ++i02) {
        ggml_quantize_chunk(GGML_TYPE_Q8_0, src.data() + i02*ne0*ne1, cpu.data() + i02*matrix_size,
                0, ne1, ne0, nullptr, nullptr);
    }

    // CPU whole-tensor single call (do_quantize single-thread path)
    ggml_quantize_chunk(GGML_TYPE_Q8_0, src.data(), cpu_whole.data(), 0, ne1*ne2, ne0, nullptr, nullptr);

    // GPU per-slice (do_quantize CUDA branch)
    cudaSetDevice(device);
    size_t nb_gpu_total = 0;
    for (int64_t i02 = 0; i02 < ne2; ++i02) {
        nb_gpu_total += ggml_cuda_quantize_q8_0(src.data() + i02*ne0*ne1, gpu.data() + i02*matrix_size, ne1, ne0);
    }
    ...
    const int64_t d_whole  = compare_buffers(tag, cpu.data(), cpu_whole.data(), cpu.size());
    const int64_t d_gpu_cpu = compare_buffers(tag, cpu.data(), gpu.data(), cpu.size());
...
int main(int argc, char ** argv) {
    ...
    printf("  [INFO] ggml_cuda_quantize_q8_0 runs on device 0 (POC); testing device %d\n", devices[0]);
    ...
    for (int di = 0; di < ntest_dev; ++di) {
        ...
        for (int64_t k = 0; k < npr_cnt; ++k) {
            for (int64_t r = 0; r < nrow_cnt; ++r) {
                ...
                test_one("random-uniform", nrows, n_per_row, fill_random_uniform, dev);
                test_one("weight-like",   nrows, n_per_row, fill_random_weight_like, dev);
                test_one("edge-cases",    std::min<int64_t>(nrows, 1024), n_per_row, fill_edge_cases, dev);
            }
        }
    }

    printf("\n--- Test: chunk-loop boundary (>1<<20 quant blocks) ---\n");
    test_one("chunk-boundary", 16385, 2048, fill_random_uniform, devices[0]);

    printf("\n--- Test: do_quantize ne[2] slice reproduction ---\n");
    test_slices(devices[0], 32,   17,   4);
    test_slices(devices[0], 512,  128,  4);
    test_slices(devices[0], 2048, 128,  4);
    test_slices(devices[0], 2048, 128, 17);

    if (big) {
        printf("\n--- Test: big tensor (token-embd scale) ---\n");
        test_one("big", 65536, 2048, fill_random_uniform, devices[0]);
    }
    if (huge) {
        printf("\n--- Test: huge tensor (Llama-3.2-1B token_embd 128256x2048) ---\n");
        test_one("huge-token_embd", 128256, 2048, fill_random_uniform, devices[0]);
    }
