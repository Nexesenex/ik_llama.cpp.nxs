...
ggml_context * create_tensors_helper::get_context_for_tensor(ggml_context * ctx, const std::string & name) {
    // When split_output_tensor is enabled, output tensors must stay in the split
    // context and must not be overridden by -ot to a monolithic per-device buffer.
    if (model.split_output_tensor) {
        if (name == "output.weight" || name == "output_norm.weight" ||
            name == "output.bias"   || name == "output_extra.weight" ||
...
bool create_tensors_helper::create_llama_tensors(const LLM_TN & tn) {
    ...
    if (model.split_output_tensor) {
        ...
        LLAMA_LOG_INFO("split_output_tensor = OFF\n");
    }
...
bool create_tensors_helper::create_qwen2_tensors(const LLM_TN & tn) {
    ...
    // output
    {
        auto out_ctx = model.split_output_tensor ? ctx_output_split : ctx_output;
        model.output_norm = create_tensor(out_ctx, tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd});
        model.output      = create_tensor(out_ctx, tn(LLM_TENSOR_OUTPUT,      "weight"), {n_embd, n_vocab}, llama_model_loader::TENSOR_NOT_REQUIRED);
        model.output_b    = create_tensor(out_ctx, tn(LLM_TENSOR_OUTPUT,      "bias"),   {n_vocab}, llama_model_loader::TENSOR_NOT_REQUIRED);
...
bool create_tensors_helper::create_qwen35moe_tensors(const LLM_TN & tn) {
    ...
    // output
    {
        ...
        auto output_mtp = create_tensor(out_ctx, "output_extra.weight", {n_embd, n_vocab}, flags);
        if (model.mtp) {
