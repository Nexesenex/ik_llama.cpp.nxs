            ...
            if (layer.ffn_gate_inp) {
                if (auto it = split_tensors.find(layer.ffn_gate_inp); it != split_tensors.end()) {
                    ...
                    if (!ffn_split.empty() && ffn_split.size() > 1) {
                        ...
                        shared_split = create_split(ggml_nrows(layer.ffn_gate_inp), -1, cur_splits, mem_used, vram_free, vram_total, ggml_nbytes(layer.ffn_gate_inp), model.split_vram_reserve_factor,
                            model.split_tensor_split_factor, model.split_vram_free_factor, model.split_usage_penalty_factor);
                    }
            ...
            if (layer.ffn_gate_inp_b) {
                if (auto it = split_tensors.find(layer.ffn_gate_inp_b); it != split_tensors.end()) {
                    ...
                    if (!ffn_split.empty() && ffn_split.size() > 1) {
                        ...
                        shared_split = create_split(ggml_nrows(layer.ffn_gate_inp_b), -1, cur_splits, mem_used, vram_free, vram_total, ggml_nbytes(layer.ffn_gate_inp_b), model.split_vram_reserve_factor,
                            model.split_tensor_split_factor, model.split_vram_free_factor, model.split_usage_penalty_factor);
                    }
            ...
            if (layer.ffn_exp_probs_b) {
                if (auto it = split_tensors.find(layer.ffn_exp_probs_b); it != split_tensors.end()) {
                    ...
                    if (!ffn_split.empty() && ffn_split.size() > 1) {
                        ...
                        shared_split = create_split(ggml_nrows(layer.ffn_exp_probs_b), -1, cur_splits, mem_used, vram_free, vram_total, ggml_nbytes(layer.ffn_exp_probs_b), model.split_vram_reserve_factor,
                            model.split_tensor_split_factor, model.split_vram_free_factor, model.split_usage_penalty_factor);
                    }
