...
static ggml_type llama_tensor_get_type(quantize_state_internal & qs, ggml_type new_type, const ggml_tensor * tensor, llama_ftype ftype) {
    ...
    if (name == tn(LLM_TENSOR_OUTPUT, "weight") || (!qs.has_output && name == tn(LLM_TENSOR_TOKEN_EMBD, "weight"))) {
        if (qs.params->output_tensor_type < GGML_TYPE_COUNT) {
            ...
            else if (ftype == LLAMA_FTYPE_MOSTLY_IQ2_K  || ftype == LLAMA_FTYPE_MOSTLY_IQ3_K   ||
            ...
            else if ((ftype == LLAMA_FTYPE_MOSTLY_IQ4_KS || ftype == LLAMA_FTYPE_MOSTLY_IQ4_KSS || ftype == LLAMA_FTYPE_MOSTLY_IQ4_KS_R4) && !qs.has_output) {
        ...
        if (name.find("attn_v.weight") != std::string::npos) {
        ...
        else if (qs.model.hparams.n_expert >= 4 && name.find("attn_k") != std::string::npos) {
            new_type = GGML_TYPE_Q4_K;
        }
        else if (qs.model.hparams.n_expert >= 4 && name.find("attn_q") != std::string::npos) {
            new_type = GGML_TYPE_Q4_K;
        }
        else if (name.find("attn_qkv.weight") != std::string::npos) {
            new_type = ftype == LLAMA_FTYPE_MOSTLY_IQ2_S || is_iq2_m ? GGML_TYPE_IQ3_XXS : GGML_TYPE_IQ2_S;
        }
        else if (name.find("ffn_down") != std::string::npos) {
            ...
            if (qs.params->attn_output_type < GGML_TYPE_COUNT) new_type = qs.params->attn_output_type;
                ...
                else if (ftype == LLAMA_FTYPE_MOSTLY_IQ2_S || is_iq2_m) new_type = GGML_TYPE_IQ3_XXS;
            }
        }
        ...
        else if ((ftype == LLAMA_FTYPE_MOSTLY_IQ4_NL_R4 || ftype == LLAMA_FTYPE_MOSTLY_IQ4_XS_R8 ||
        ...
        else if (ftype == LLAMA_FTYPE_MOSTLY_IQ5_K && qs.model.hparams.n_gqa() >= 2) {
            new_type = GGML_TYPE_IQ6_K;
        }
        else if (ftype == LLAMA_FTYPE_MOSTLY_IQ5_K_R4 && qs.model.hparams.n_gqa() >= 2) {
            new_type = GGML_TYPE_IQ6_K;
        }
        ...
        else if (ftype == LLAMA_FTYPE_MOSTLY_Q6_0) {
            new_type = GGML_TYPE_Q5_0;
        }
        else if (ftype == LLAMA_FTYPE_MOSTLY_IQ3_K && qs.has_imatrix) {
            new_type = GGML_TYPE_IQ2_K;
        }
        else if (ftype == LLAMA_FTYPE_MOSTLY_IQ4_K) {
            new_type = GGML_TYPE_IQ3_K;
        }
        else if (ftype == LLAMA_FTYPE_MOSTLY_IQ5_K) {
            new_type = GGML_TYPE_IQ4_K;
        }
        else if (ftype == LLAMA_FTYPE_MOSTLY_IQ6_K) {
            new_type = GGML_TYPE_IQ5_K;
        }
    } else if (name.find("ffn_down") != std::string::npos) {
    ...
    if (new_type == GGML_TYPE_Q2_K    || new_type == GGML_TYPE_Q3_K    || new_type == GGML_TYPE_Q4_K   ||
        ...
        int ny = tensor->ne[1];
        if (nx % QK_K != 0) {
            LLAMA_LOG_WARN("\n\n%s : tensor cols %d x %d not divisible by %d, required for %s", __func__, nx, ny, QK_K, ggml_type_name(new_type));
