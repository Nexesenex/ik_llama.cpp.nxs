    ...
    llama_model_loader(const std::string & fname, bool use_mmap, bool check_tensors, const struct llama_model_kv_override * param_overrides_p) {
        ...
        // TODO: make optional
        {
            ...
            switch (type_max) {
                ...
                case GGML_TYPE_IQ4_KNN: ftype = LLAMA_FTYPE_MOSTLY_IQ4_KNN; break;
...
static std::string llama_model_ftype_name(llama_ftype ftype) {
    ...
    switch (ftype) {
        ...
        case LLAMA_FTYPE_MOSTLY_IQ4_KNN:  return "IQ4_KNN - 4.0 bpw";
...
static ggml_type llama_tensor_get_type(quantize_state_internal & qs, ggml_type new_type, const ggml_tensor * tensor, llama_ftype ftype) {
    ...
    if (name == tn(LLM_TENSOR_OUTPUT, "weight") || (!qs.has_output && name == tn(LLM_TENSOR_TOKEN_EMBD, "weight"))) {
        if (qs.params->output_tensor_type < GGML_TYPE_COUNT) {
            ...
            if (arch == LLM_ARCH_FALCON || nx % QK_K != 0) {
                ...
                new_type = !qs.has_output ? GGML_TYPE_IQ4_K : GGML_TYPE_Q5_K;
                      ...
                      ftype == LLAMA_FTYPE_MOSTLY_IQ4_KS || ftype == LLAMA_FTYPE_MOSTLY_IQ4_KSS || ftype == LLAMA_FTYPE_MOSTLY_IQ4_KNN) && !qs.has_output) {
        ...
        if      (ftype == LLAMA_FTYPE_MOSTLY_Q2_K) {
            ...
            new_type = qs.i_attention_wv < 2 ? GGML_TYPE_Q5_K : GGML_TYPE_Q4_K;
                  ...
                  ftype == LLAMA_FTYPE_MOSTLY_IQ4_KNN) && qs.model.hparams.n_gqa() >= 2) {
            new_type = GGML_TYPE_IQ5_K;
        }
        ...
        if      (ftype == LLAMA_FTYPE_MOSTLY_Q2_K) new_type = GGML_TYPE_Q3_K;
            ...
            if (arch == LLM_ARCH_FALCON) {
                ...
                if (use_more_bits(i_layer, n_layer)) new_type = GGML_TYPE_Q6_K;
                    ...
                    ftype == LLAMA_FTYPE_MOSTLY_IQ4_KS || ftype == LLAMA_FTYPE_MOSTLY_IQ4_KSS || ftype == LLAMA_FTYPE_MOSTLY_IQ4_KNN) && !qs.has_imatrix) {
            new_type = GGML_TYPE_Q5_K;
        }
    ...
    if (new_type == GGML_TYPE_Q2_K    || new_type == GGML_TYPE_Q3_K    || new_type == GGML_TYPE_Q4_K   ||
        ...
        new_type == GGML_TYPE_IQ2_KS  || new_type == GGML_TYPE_IQ4_KSS || new_type == GGML_TYPE_IQ4_KNN) {
        int nx = tensor->ne[0];
        int ny = tensor->ne[1];
        if (nx % QK_K != 0) {
...
static void llama_model_quantize_internal(const std::string & fname_inp, const std::string & fname_out, const llama_model_quantize_params * params) {
    ...
    switch (params->ftype) {
        ...
        case LLAMA_FTYPE_MOSTLY_IQ4_KNN: default_type = GGML_TYPE_IQ4_KNN; break;
