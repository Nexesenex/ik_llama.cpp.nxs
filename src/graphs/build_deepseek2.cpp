    ...
    for (int il = 0; il < n_active_layers; ++il) {
        ...
        if ((uint32_t) il < hparams.n_layer_dense_lead) {
            ...
            cur = llm_build_std_moe_ffn(ctx0, lctx, model.layers[il].ffn_norm, cur,
                    ...
                    LLM_FFN_SILU, cb, il, gf, /*add_input=*/true, model.layers[il].ffn_up_gate_exps, nullptr, nullptr, nullptr,
            ...
            if (model.layers[il].ffn_up_gate_shexp) {
                ...
                ffn_shexp = ggml_mul(ctx0, up_part, gate_part);
                cb(ffn_shexp, "ffn_shexp_up_gated", il);
                ffn_shexp = llm_build_lora_mm(lctx, ctx0, model.layers[il].ffn_down_shexp, ffn_shexp);
...
struct ggml_tensor * llm_build_context::build_deepseek2_mtp(
    ...
    // MoE FFN (MTP layer is always in the MoE range, not dense)
    {
        ...
        // Shared Expert FFN
        ggml_tensor * ffn_shexp;
        if (mtp_layer.ffn_up_gate_shexp) {
            auto n_ff_shexp = mtp_layer.ffn_up_gate_shexp->ne[1] / 2;
            auto combined = llm_build_lora_mm(lctx, ctx0, mtp_layer.ffn_up_gate_shexp, cur);
            cb(combined, "ffn_shexp_combined", il);
            auto type_size = ggml_type_size(combined->type);
            auto gate_part = ggml_view_2d(ctx0, combined, n_ff_shexp, combined->ne[1], combined->nb[1], 0);
            auto up_part = ggml_view_2d(ctx0, combined, n_ff_shexp, combined->ne[1], combined->nb[1], n_ff_shexp * type_size);
            ...
            ffn_shexp = ggml_mul(ctx0, up_part, gate_part);
            cb(ffn_shexp, "ffn_shexp_up_gated", il);
            ffn_shexp = llm_build_lora_mm(lctx, ctx0, mtp_layer.ffn_down_shexp, ffn_shexp);
            cb(ffn_shexp, "ffn_shexp", il);
        } else {
            ffn_shexp = llm_build_ffn(ctx0, lctx, nullptr, cur,
