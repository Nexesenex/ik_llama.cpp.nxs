    ...
    if (!model.layers[il].wqkv && !model.layers[il].wqk && cparams.flash_attn &&
        ...
        if (kv_self.k_l[il]->extra && kv_self.v_l[il]->extra) {
            ...
            for (int id = 0; id < wq->n_device; ++id) {
                ...
                // swa-ring and on-device SWA compaction are disjoint: ring layers use the
                // window-sized ring (offset 0), compacted layers use the window view.
                const int32_t n_kv_eff  = ring ? (int32_t) kv_size_l : n_kv_view;
                const int32_t kv_off_eff = ring ? 0 : kv_view_offset;
                auto knb1 = k_row_size*n_head_kv;
                auto k = ggml_view_3d(ctx0, split_kl, n_embd_head_k, n_kv_eff, n_head_kv,
                             knb1, k_row_size, kv_off_eff*knb1);
                cb(k, "k", il_cb);

                auto v_row_size = ggml_row_size(split_vl->type, n_embd_head_v);
                auto vnb1 = ggml_row_size(split_vl->type, wv->splits[id]->ne[1]);
                auto v = ggml_view_3d(ctx0, split_vl, n_embd_head_v, n_kv_eff, n_head_kv,
                ...
                if (n_swa > 0 && can_use_kv_swa_reduction(cparams, kv_self) && !lctx.kv_self.cells_disordered) {
                    ((int32_t *)cur->op_params)[4] = n_swa;
                }
                // Some models produced NaNs/gibberish when FA is computed with f16 precision on CUDA
                if (should_use_f32_precision) {
