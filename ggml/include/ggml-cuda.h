...
// -p-warm-i / -warmstream). Applies to all three warmup functions (mma, fma, mem):
// on each WDDM GPU the burst fires on every N-th TG batch instead of every batch.
// intervals[] maps positionally to non-TCC (WDDM) GPUs; a single value broadcasts to
// every WDDM GPU (bare --poller-warmup-interval = 1); missing values use the default 1
// (fire every batch, the historical behavior).
GGML_API GGML_CALL void ggml_backend_cuda_set_poller_warmup_interval(const int * intervals, int n);

// First TG token at which the warmup burst fires (--poller-warmup-start N[,N,...], aliases
// -p-warm-s / -streamsource). Applies to all three warmup functions (mma, fma, mem):
// on each WDDM GPU the burst first fires on the N-th TG batch (token) of a phase instead of
// the historical default second token. starts[] maps positionally to non-TCC (WDDM) GPUs;
// a single value broadcasts to every WDDM GPU (bare --poller-warmup-start = 2, fire on the
// second TG token, the historical skip-first-batch behavior); missing values use the default 2.
GGML_API GGML_CALL void ggml_backend_cuda_set_poller_warmup_start(const int * starts, int n);
...
// 0 = disabled (no burst on that GPU), 100 = full grid (16 blocks/SM). A single value
// broadcasts to every WDDM GPU (bare --poller-fma-occupancy = 50, the default); more values
// map positionally; missing values use the given default (50). Applies to the warmup, the
// decode-solicited activity probes and the autonomous ping thread.
GGML_API GGML_CALL void ggml_backend_cuda_set_poller_fma_occupancy(const float * occ, int n, float default_occ);
...
// 0 = disabled (no burst on that GPU), 100 = full grid (16 blocks/SM). A single value
// broadcasts to every WDDM GPU (bare --poller-mma-occupancy = 50, the default); more values
// map positionally; missing values use the given default (50). Applies to the warmup,
// activity probes and ping thread.
GGML_API GGML_CALL void ggml_backend_cuda_set_poller_mma_occupancy(const float * occ, int n, float default_occ);
...
// percentage: 0 = disabled (no burst on that GPU), 100 = full 2 MiB buffer per pass. A
// single value broadcasts to every WDDM GPU (bare --poller-mem-occupancy = 25, the default);
// more values map positionally; missing values use the given default (25). Applies to the
// warmup, activity probes and ping.
GGML_API GGML_CALL void ggml_backend_cuda_set_poller_mem_occupancy(const float * occ, int n, float default_occ);
...
// Currently implemented: Q8_0, Q4_0 (ref), Q4_0 with importance matrix.
// Byte-for-byte identical to the corresponding quantize_row_*_ref CPU
// implementation (and to the imatrix make_qx_quants path for Q4_0).
