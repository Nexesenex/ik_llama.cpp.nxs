...
namespace {
inline std::vector<float> & get_f32_work_buffer(size_t size) {
    thread_local std::vector<float> buffer;
    if (buffer.size() < size) buffer.resize(size);
    return buffer;
}
inline void iqk_f32_add(int n, const float * x, float * y) {
    int i = 0;
#if defined __AVX2__
    for (; i + 7 < n; i += 8) {
...
void iqk_reduce(struct ggml_tensor * dst, int ith, int nth) {
    ...
    GGML_ASSERT(src0);
    GGML_ASSERT(src0->type == dst->type);
    for (int i = 1; i < nhave; ++i) {
        GGML_ASSERT(dst->src[i]);
    ...
    if (dst->type == GGML_TYPE_F32) {
        for (int ir = first; ir < last; ++ir) {
            auto acc = (float *)((char *)dst->src[0]->data + ir*dst->src[0]->nb[1]);
            for (int i = 1; i < nhave; ++i) {
                auto x = (const float *)((const char *)dst->src[i]->data + ir*dst->src[i]->nb[1]);
                iqk_f32_add(ne00, x, acc);
            }
            for (int i = 1; i < nhave; ++i) {
                auto y = (float *)((char *)dst->src[i]->data + ir*dst->src[i]->nb[1]);
                std::memcpy(y, acc, ne00*sizeof(float));
            }
        }
        return;
    }

    auto traits = ggml_internal_get_type_traits(dst->type);
    GGML_ASSERT(traits.to_float != nullptr);
    GGML_ASSERT(traits.from_float != nullptr);

    auto & work = get_f32_work_buffer(2*ne00);
    float * acc = work.data();
    float * tmp = work.data() + ne00;

    for (int ir = first; ir < last; ++ir) {
        std::memset(acc, 0, ne00*sizeof(float));
        for (int i = 0; i < nhave; ++i) {
            auto x = (const char *)dst->src[i]->data + ir*dst->src[i]->nb[1];
            traits.to_float(x, tmp, ne00);
            iqk_f32_add(ne00, tmp, acc);
        }
        for (int i = 0; i < nhave; ++i) {
            auto y = (char *)dst->src[i]->data + ir*dst->src[i]->nb[1];
            traits.from_float(acc, y, ne00);
        }
    }
}
