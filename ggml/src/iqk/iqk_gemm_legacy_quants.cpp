...
#if defined(HAVE_VNNIINT8)
    auto sumi = _mm256_setzero_si256();
    sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(yl, 0x00), v[0]);
    sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(yl, 0x55), v[1]);
    sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(yl, 0xaa), v[2]);
    sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(yl, 0xff), v[3]);
    sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(yh, 0x00), v[4]);
    sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(yh, 0x55), v[5]);
    sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(yh, 0xaa), v[6]);
    sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(yh, 0xff), v[7]);
#else
    auto sumi = _mm256_setzero_si256();
...
static void mul_mat_mxfp4_r8_q8_2_avx2(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
    ...
    Q8<nrc_y, block_q8_2_x4> q8(info);
    auto m4 = _mm256_set1_epi8(0xf);
    auto mxfp4_lut = load_unsigned_mxfp4_values_256();
    int nb = n / QK_MXFP4;
    __m256i v[8];
    auto load_e8m0_scales = [](const uint8_t * e) {
        float buf[8];
        for (int j = 0; j < 8; ++j) buf[j] = GGML_E8M0_TO_FP32_HALF(e[j]);
        return _mm256_loadu_ps(buf);
    };
    auto apply_mxfp4_lut = [&mxfp4_lut](__m256i * v) {
        for (int i = 0; i < 8; ++i) v[i] = _mm256_shuffle_epi8(mxfp4_lut, v[i]);
    };
    if constexpr (nrc_y == 1) {
        union { __m256 vec; float val[8]; } helper;
        for (int ix = 0; ix < nrc_x; ix += 8) {
            const block_mxfp4_r8 * mx = (const block_mxfp4_r8 *)((const char *)vx + ix*bx);
            auto acc1 = _mm256_setzero_ps();
            auto acc2 = _mm256_setzero_ps();
            for (int ib4 = 0; ib4 < nb/4; ++ib4) {
                helper.vec = convert_scales((const uint16_t *)q8.y[0][ib4].d);
                for (int k = 0; k < 4; ++k) {
                    auto scales = load_e8m0_scales(mx[4*ib4+k].e);
                    prepare_q4_0_quants_avx2(mx[4*ib4+k].qs, v, m4);
                    apply_mxfp4_lut(v);
                    auto sumi = accum_q4_0_quants(v, q8.y[0][ib4].qs+32*k);
                    auto d4d8 = _mm256_mul_ps(scales, _mm256_set1_ps(helper.val[k]));
                    acc1 = _mm256_fmadd_ps(d4d8, _mm256_cvtepi32_ps(sumi), acc1);
            ...
            for (int ib = 4*(nb/4); ib < nb; ++ib) {
                auto qy = (const block_q8_2 *)q8.y[0];
                auto scales = load_e8m0_scales(mx[ib].e);
                prepare_q4_0_quants_avx2(mx[ib].qs, v, m4);
                apply_mxfp4_lut(v);
                auto sumi = accum_q4_0_quants(v, qy[ib].qs);
                auto [d8, m8] = ScaleHelperQ8_2::prepare1(qy + ib);
                auto d4d8 = _mm256_mul_ps(scales, _mm256_set1_ps(d8));
            ...
            for (int ib4 = 0; ib4 < nb/4; ++ib4) {
                {
                    __m256 d4[4];
                    for (int k = 0; k < 4; ++k) {
                        d4[k] = load_e8m0_scales(mx[4*ib4+k].e);
                    }
                    for (int iy = 0; iy < nrc_y; ++iy) {
                        auto scales = convert_scales((const uint16_t *)q8.y[iy][ib4].d);
                        _mm256_storeu_ps(d8 + 8*iy, scales);
                        auto m4s = _mm256_extractf128_ps(scales, 1);
                        auto m8 = MM256_SET1_M128(m4s);
                        auto sumf = _mm256_mul_ps(d4[0], _mm256_shuffle_ps(m8, m8, 0x00));
                        sumf = _mm256_fmadd_ps(d4[1], _mm256_shuffle_ps(m8, m8, 0x55), sumf);
                        sumf = _mm256_fmadd_ps(d4[2], _mm256_shuffle_ps(m8, m8, 0xaa), sumf);
                        sumf = _mm256_fmadd_ps(d4[3], _mm256_shuffle_ps(m8, m8, 0xff), sumf);
...
static void mul_mat_mxfp4_r8_q8_2(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
    ...
    Q8<nrc_y, block_q8_2_x4> q8(info);
    auto m4 = _mm512_set1_epi8(0xf);
    auto mxfp4_lut = load_unsigned_mxfp4_values_512();
    int nb = n / QK_MXFP4;
    __m512  acc[2*nrc_y] = {};
    __m512i qx[8];
    auto prepare = [&qx, &m4, &mxfp4_lut] (const block_mxfp4_r8& mxl, const block_mxfp4_r8& mxh) {
        float buf_lo[8], buf_hi[8];
        for (int j = 0; j < 8; ++j) { buf_lo[j] = GGML_E8M0_TO_FP32_HALF(mxl.e[j]); buf_hi[j] = GGML_E8M0_TO_FP32_HALF(mxh.e[j]); }
        auto s_lo = _mm256_loadu_ps(buf_lo);
        auto s_hi = _mm256_loadu_ps(buf_hi);
        auto scales = _mm512_insertf32x8(_mm512_castps256_ps512(s_lo), s_hi, 1);
        for (int j = 0; j < 4; ++j) {
            auto bits = _mm512_inserti32x8(_mm512_castsi256_si512(_mm256_loadu_si256((const __m256i *)mxl.qs+j)),
                    _mm256_loadu_si256((const __m256i *)mxh.qs+j), 1);
            qx[j+0] = _mm512_shuffle_epi8(mxfp4_lut, _mm512_and_si512(bits, m4));
            qx[j+4] = _mm512_shuffle_epi8(mxfp4_lut, _mm512_and_si512(_mm512_srli_epi16(bits, 4), m4));
        }
        return scales;
    };
    auto dot = [&qx] (const int8_t * qy) {
    ...
    float d8[8*nrc_y];
    for (int ix = 0; ix < nrc_x; ix += 16) {
        const block_mxfp4_r8 * mxl = (const block_mxfp4_r8 *)((const char *)vx + (ix+0)*bx);
        const block_mxfp4_r8 * mxh = (const block_mxfp4_r8 *)((const char *)vx + (ix+8)*bx);
        for (int ib4 = 0; ib4 < nb/4; ++ib4) {
            for (int iy = 0; iy < nrc_y; ++iy) {
                _mm256_storeu_ps(d8+8*iy, convert_scales((const uint16_t *)q8.y[iy][ib4].d));
            }
            for (int k = 0; k < 4; ++k) {
                auto scales = prepare(mxl[4*ib4+k], mxh[4*ib4+k]);
                for (int iy = 0; iy < nrc_y; ++iy) {
                    auto sumi = dot(q8.y[iy][ib4].qs+32*k);
                    auto dy = _mm512_set1_ps(d8[8*iy+k]);
                    acc[2*iy+0] = _mm512_fmadd_ps(_mm512_mul_ps(scales, dy), _mm512_cvtepi32_ps(sumi), acc[2*iy+0]);
        ...
        for (int ib = 4*(nb/4); ib < nb; ++ib) {
            auto scales = prepare(mxl[ib], mxh[ib]);
            for (int iy = 0; iy < nrc_y; ++iy) {
                auto qy = (const block_q8_1 *)q8.y[iy];
                auto sumi = dot(qy[ib].qs);
                auto [d8, m8] = ScaleHelperQ8_2::prepare1(qy + ib);
    ...
    if (nrc_x % 16 != 0) {
        int ix = nrc_x - 8;
        const block_mxfp4_r8 * mxl = (const block_mxfp4_r8 *)((const char *)vx + ix*bx);
        block_mxfp4_r8 mxh_zero = {};
        for (int ib4 = 0; ib4 < nb/4; ++ib4) {
            for (int iy = 0; iy < nrc_y; ++iy) {
                _mm256_storeu_ps(d8+8*iy, convert_scales((const uint16_t *)q8.y[iy][ib4].d));
            }
            for (int k = 0; k < 4; ++k) {
                auto scales = prepare(mxl[4*ib4+k], mxh_zero);
                for (int iy = 0; iy < nrc_y; ++iy) {
                    auto sumi = dot(q8.y[iy][ib4].qs+32*k);
                    auto dy = _mm512_set1_ps(d8[8*iy+k]);
                    acc[2*iy+0] = _mm512_fmadd_ps(_mm512_mul_ps(scales, dy), _mm512_cvtepi32_ps(sumi), acc[2*iy+0]);
        ...
        for (int ib = 4*(nb/4); ib < nb; ++ib) {
            auto scales = prepare(mxl[ib], mxh_zero);
            for (int iy = 0; iy < nrc_y; ++iy) {
                auto qy = (const block_q8_1 *)q8.y[iy];
                auto sumi = dot(qy[ib].qs);
                auto [d8l, m8] = ScaleHelperQ8_2::prepare1(qy + ib);
...
// MulMat kernel for Q6_1_R4 (4 logical rows per physical block) × Q8_2 format.
// Q6_1 has d (scale) and m (min) per block. Values are unsigned [0, 63].
// Dot: d * sum(q6_i * q8_i) + m * sum(q8_i)
// The q8_i and sum(q8_i) come from Q8_2 format (d8 = scale, m8 = scale * sum(q8)).
// Q6_0 uses -16*d for the offset (centered), Q6_1 uses m.
template <int nrc_y>
static void mul_mat_q6_1_r4_q8_2_avx2(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
    GGML_ASSERT(nrc_x%4 == 0);
    Q8<nrc_y, block_q8_2_x4> q8(info);
    auto m4 = _mm256_set1_epi8(0xf);
    auto m6 = _mm256_set1_epi8(0x30);
    ...
    float d8[8*nrc_y];
    __m256i qx[4];
    auto prepare = [&qx, &m4, &m6] (const block_q6_1_r4& iq6, __m256& m_scales) {
        auto d128 = _mm_cvtph_ps(_mm_loadu_si128((const __m128i *)iq6.d));
        auto m128 = _mm_cvtph_ps(_mm_loadu_si128((const __m128i *)iq6.m));
        auto d_scales = MM256_SET1_M128(d128);
        m_scales = MM256_SET1_M128(m128);
        auto bits1 = _mm256_loadu_si256((const __m256i *)iq6.qs+0);
        auto bits2 = _mm256_loadu_si256((const __m256i *)iq6.qs+1);
        auto hbits = _mm256_loadu_si256((const __m256i *)iq6.qh);
        qx[0] = _mm256_or_si256(_mm256_and_si256(bits1, m4), _mm256_and_si256(_mm256_slli_epi16(hbits, 4), m6));
    ...
    auto mscale = _mm256_set_m128(_mm_set1_ps(-16.f), _mm_set1_ps(1.f));
    for (int ix = 0; ix < nrc_x; ix += 4) {
        const block_q6_1_r4 * iq6 = (const block_q6_1_r4 *)((const char *)vx + ix*bx);
        for (int ib = 0; ib < nb/4; ++ib) {
            for (int iy = 0; iy < nrc_y; ++iy) {
                auto scales = convert_scales((const uint16_t *)q8.y[iy][ib].d);
                _mm256_storeu_ps(d8 + 8*iy, _mm256_mul_ps(scales, mscale));
            }
            for (int k = 0; k < 4; ++k) {
                __m256 m_scales;
                auto d_scales = prepare(iq6[4*ib+k], m_scales);
                for (int iy = 0; iy < nrc_y; ++iy) {
                    auto sumi = dot(_mm256_loadu_si256((const __m256i*)q8.y[iy][ib].qs+k));
                    auto d4d8 = _mm256_mul_ps(d_scales, _mm256_set1_ps(d8[8*iy+k]));
                    acc[iy] = _mm256_fmadd_ps(d4d8, _mm256_cvtepi32_ps(sumi), acc[iy]);
                    acc[iy] = _mm256_fmadd_ps(m_scales, _mm256_set1_ps(d8[8*iy+k+4] * (-1.f/16.f)), acc[iy]);
                }
            }
        }
        for (int ib = 4*(nb/4); ib < nb; ++ib) {
            __m256 m_scales;
            auto d_scales = prepare(iq6[ib], m_scales);
            for (int iy = 0; iy < nrc_y; ++iy) {
                auto qy = (const block_q8_2 *)q8.y[iy];
                auto sumi = dot(_mm256_loadu_si256((const __m256i*)qy[ib].qs));
                auto [d8v, m8v] = ScaleHelperQ8_2::prepare1(qy + ib);
                auto d4d8 = _mm256_mul_ps(d_scales, _mm256_set1_ps(d8v));
                acc[iy] = _mm256_fmadd_ps(d4d8, _mm256_cvtepi32_ps(sumi), acc[iy]);
                acc[iy] = _mm256_fmadd_ps(m_scales, _mm256_set1_ps(m8v), acc[iy]);
            }
        }
        for (int iy = 0; iy < nrc_y; ++iy) {
            auto sum = _mm_add_ps(_mm256_castps256_ps128(acc[iy]), _mm256_extractf128_ps(acc[iy], 1));
...
#ifdef HAVE_FANCY_SIMD
template <int nrc_y>
static void mul_mat_q6_1_r4_q8_2(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
    mul_mat_q6_1_r4_q8_2_avx2<nrc_y>(n, vx, bx, info, nrc_x);
}
#else
template <int nrc_y>
static void mul_mat_q6_1_r4_q8_2(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
...
// Convert Q6_1_R4 (4 logical rows per physical row) to Q8_1_R8
static void iqk_convert_q6_1_r4_q8_1_r8(int n, const void * vx, size_t bx, void * vy, int nrc_x) {
    GGML_ASSERT(n%32 == 0);
    GGML_ASSERT(nrc_x%2 == 0);

    int nb = n/32; // Q6_1 blocks per logical row
    block_q8_1_r8 * y = (block_q8_1_r8 *)vy;

    Q6_1_Dequantizer<block_q6_1> deq;
    uint32_t block[8];

    for (int ix = 0; ix < nrc_x; ix += 2) {
        // 2 physical R4 rows = 8 logical Q6_1 rows → 1 Q8_1_R8 row
        const block_q6_1 * x8[8];
        for (int k = 0; k < 8; ++k) {
            int phys = ix + k/4;
            int sub  = k % 4;
            x8[k] = (const block_q6_1 *)((const char *)vx + phys*bx) + sub*nb;
        }
        for (int i = 0; i < nb; ++i) {
            for (int k = 0; k < 8; ++k) {
                y[i].d[k+0] = x8[k][i].d;
...
bool iqk_convert_legacy_quants_q8_r8(int type, int n, const void * vx, size_t bx, void * vy, int nrc_x) {
    switch (type) {
        ...
        case GGML_TYPE_Q6_1_R4: iqk_convert_q6_1_r4_q8_1_r8(n, vx, bx, vy, nrc_x); break;
...
bool iqk_set_kernels_legacy_quants(int ne00, int typeA, int typeB, std::array<mul_mat_t, IQK_MAX_NY>& kernels, mul_mat_t& func16) {
    ...
    switch (typeA) {
        ...
        case GGML_TYPE_Q6_1_R4:
            IQK_SET_MUL_MAT_FUNCTIONS(mul_mat_q6_1_r4_q8_2, kernels)
            break;
...
bool iqk_convert_legacy_quants_q8_r8(int type, int n, const void * vx, size_t bx, void * vy, int nrc_x) {
    switch (type) {
        ...
        case GGML_TYPE_Q6_1_R4: iqk_convert_qX_1_q8_1_r8<block_q6_1, DeqQ61>(n, vx, bx, vy, nrc_x); break;
