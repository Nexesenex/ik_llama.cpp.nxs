...
struct DotHelper {
    ...
    inline __m256i dot(__m256i x, __m256i y) const {
        ...
        return ggml_mm256_dpbsud_epi32(_mm256_setzero_si256(), y, x);
#else
...
template <typename Q8, typename Q8x4, typename Dot, bool can_pack = true> struct Sum4 {
    ...
    inline __m256i compute(const __m256i * qx, const Q8 * y) const {
        ...
        if constexpr (can_pack) {
...
#if defined(HAVE_VNNIINT8)
            const __m256i t0 = ggml_mm256_dpbsud_epi32(zero, y0, qx[0]);
            const __m256i t1 = ggml_mm256_dpbsud_epi32(zero, y1, qx[1]);
            const __m256i t2 = ggml_mm256_dpbsud_epi32(zero, y2, qx[2]);
            const __m256i t3 = ggml_mm256_dpbsud_epi32(zero, y3, qx[3]);
#else
            ...
            const __m256i y3 = _mm256_loadu_si256((const __m256i *)y4->qs + 3);
#if defined(HAVE_VNNIINT8)
            const __m256i p0 = ggml_mm256_dpbsud_epi32(zero, y0, qx[0]);
            const __m256i p1 = ggml_mm256_dpbsud_epi32(zero, y1, qx[1]);
            const __m256i p2 = ggml_mm256_dpbsud_epi32(zero, y2, qx[2]);
            const __m256i p3 = ggml_mm256_dpbsud_epi32(zero, y3, qx[3]);
#else
...
#if defined(HAVE_VNNI256) || defined(HAVE_VNNIINT8)
#if defined(HAVE_VNNIINT8)
        auto s01 = ggml_mm256_dpbsud_epi32(ggml_mm256_dpbsud_epi32(_mm256_setzero_si256(),
            _mm256_loadu_si256((const __m256i *)y4->qs+0), qx[0]),
            _mm256_loadu_si256((const __m256i *)y4->qs+1), qx[1]);
        auto s23 = ggml_mm256_dpbsud_epi32(ggml_mm256_dpbsud_epi32(_mm256_setzero_si256(),
            _mm256_loadu_si256((const __m256i *)y4->qs+2), qx[2]),
            _mm256_loadu_si256((const __m256i *)y4->qs+3), qx[3]);
...
inline __m256i accum_mxfp4_quants(const __m256i * v, const int8_t * qs) {
    ...
    auto sumi1 = _mm256_setzero_si256();
    auto sumi2 = _mm256_setzero_si256();
    sumi1 = ggml_mm256_dpbssd_epi32(sumi1, v[0], _mm256_set1_epi32(*((const int32_t *)(qs +  0))));
    sumi2 = ggml_mm256_dpbssd_epi32(sumi2, v[1], _mm256_set1_epi32(*((const int32_t *)(qs +  4))));
    sumi1 = ggml_mm256_dpbssd_epi32(sumi1, v[2], _mm256_set1_epi32(*((const int32_t *)(qs +  8))));
    sumi2 = ggml_mm256_dpbssd_epi32(sumi2, v[3], _mm256_set1_epi32(*((const int32_t *)(qs + 12))));
    sumi1 = ggml_mm256_dpbssd_epi32(sumi1, v[4], _mm256_set1_epi32(*((const int32_t *)(qs + 16))));
    sumi2 = ggml_mm256_dpbssd_epi32(sumi2, v[5], _mm256_set1_epi32(*((const int32_t *)(qs + 20))));
    sumi1 = ggml_mm256_dpbssd_epi32(sumi1, v[6], _mm256_set1_epi32(*((const int32_t *)(qs + 24))));
    sumi2 = ggml_mm256_dpbssd_epi32(sumi2, v[7], _mm256_set1_epi32(*((const int32_t *)(qs + 28))));
    auto sumi = _mm256_add_epi32(sumi1, sumi2);
#elif defined(HAVE_VNNI256)
    auto sumi1 = _mm256_setzero_si256();
    auto sumi2 = _mm256_setzero_si256();
...
// Mirrors iqk_test_gemm_iq4_xs_r8 in iqk_gemm_kquants.cpp.
extern "C" void iqk_test_gemm_mxfp4_r8(int n, const void * vx, size_t bx,
                                        const DataInfo& info, int nrc_x, int nrc_y) {
    switch (nrc_y) {
        case 1: mul_mat_mxfp4_r8_q8_2_avx2<1>(n, vx, bx, info, nrc_x); break;
        case 2: mul_mat_mxfp4_r8_q8_2_avx2<2>(n, vx, bx, info, nrc_x); break;
        case 3: mul_mat_mxfp4_r8_q8_2_avx2<3>(n, vx, bx, info, nrc_x); break;
        case 4: mul_mat_mxfp4_r8_q8_2_avx2<4>(n, vx, bx, info, nrc_x); break;
        case 5: mul_mat_mxfp4_r8_q8_2_avx2<5>(n, vx, bx, info, nrc_x); break;
        case 6: mul_mat_mxfp4_r8_q8_2_avx2<6>(n, vx, bx, info, nrc_x); break;
        case 7: mul_mat_mxfp4_r8_q8_2_avx2<7>(n, vx, bx, info, nrc_x); break;
        default: mul_mat_mxfp4_r8_q8_2_avx2<8>(n, vx, bx, info, nrc_x); break;
    }
}
...
static void mul_mat_q5_0_r4_q8_2_avx2(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
    ...
    auto dot = [&qx] (__m256i y) {
        auto sumi = _mm256_setzero_si256();
#if defined(HAVE_VNNIINT8)
        sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(y, 0x00), qx[0]);
        sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(y, 0x55), qx[1]);
        sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(y, 0xaa), qx[2]);
        sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(y, 0xff), qx[3]);
#else
...
static void mul_mat_q6_0_r4_q8_2_avx2(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
    ...
    auto dot = [&qx] (__m256i y) {
#if defined(HAVE_VNNIINT8)
        auto sumi = ggml_mm256_dpbsud_epi32(_mm256_setzero_si256(), _mm256_shuffle_epi32(y, 0x00), qx[0]);
        sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(y, 0x55), qx[1]);
        sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(y, 0xaa), qx[2]);
        sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_shuffle_epi32(y, 0xff), qx[3]);
#else
...
#ifdef HAVE_VNNIINT8
        auto sumi = _mm256_setzero_si256();
        sumi = ggml_mm256_dpbssd_epi32(sumi, qx[0], _mm256_set1_epi32(*((const int32_t *)(qy +  0))));
        sumi = ggml_mm256_dpbssd_epi32(sumi, qx[1], _mm256_set1_epi32(*((const int32_t *)(qy +  4))));
        sumi = ggml_mm256_dpbssd_epi32(sumi, qx[2], _mm256_set1_epi32(*((const int32_t *)(qy +  8))));
        sumi = ggml_mm256_dpbssd_epi32(sumi, qx[3], _mm256_set1_epi32(*((const int32_t *)(qy + 12))));
        return sumi;
#elif defined(HAVE_VNNI256)
        auto sumi = _mm256_setzero_si256();
        sumi = ggml_mm256_dpbusd_epi32(sumi, sx[0], _mm256_sign_epi8(_mm256_set1_epi32(*((const int32_t *)(qy +  0))), qx[0]));
        sumi = ggml_mm256_dpbusd_epi32(sumi, sx[1], _mm256_sign_epi8(_mm256_set1_epi32(*((const int32_t *)(qy +  4))), qx[1]));
        sumi = ggml_mm256_dpbusd_epi32(sumi, sx[2], _mm256_sign_epi8(_mm256_set1_epi32(*((const int32_t *)(qy +  8))), qx[2]));
        sumi = ggml_mm256_dpbusd_epi32(sumi, sx[3], _mm256_sign_epi8(_mm256_set1_epi32(*((const int32_t *)(qy + 12))), qx[3]));
        return sumi;
#else
        auto sumi1 = _mm256_add_epi32(
                _mm256_madd_epi16(m1, _mm256_maddubs_epi16(sx[0], _mm256_sign_epi8(_mm256_set1_epi32(*((const int32_t *)(qy +  0))), qx[0]))),
                _mm256_madd_epi16(m1, _mm256_maddubs_epi16(sx[1], _mm256_sign_epi8(_mm256_set1_epi32(*((const int32_t *)(qy +  4))), qx[1])))
        );
        auto sumi2 = _mm256_add_epi32(
                _mm256_madd_epi16(m1, _mm256_maddubs_epi16(sx[2], _mm256_sign_epi8(_mm256_set1_epi32(*((const int32_t *)(qy +  8))), qx[2]))),
                _mm256_madd_epi16(m1, _mm256_maddubs_epi16(sx[3], _mm256_sign_epi8(_mm256_set1_epi32(*((const int32_t *)(qy + 12))), qx[3])))
        );
        return _mm256_add_epi32(sumi1, sumi2);
#endif
    };
...
inline __m256i qx_r8_q8_dot_product(const __m256i * qx, const int8_t * y) {
    ...
    auto sumi1 = _mm256_setzero_si256();
    auto sumi2 = _mm256_setzero_si256();
    sumi1 = ggml_mm256_dpbsud_epi32(sumi1, _mm256_set1_epi32(*((const int32_t *)(y +  0))), qx[0]);
    sumi2 = ggml_mm256_dpbsud_epi32(sumi2, _mm256_set1_epi32(*((const int32_t *)(y +  4))), qx[1]);
    sumi1 = ggml_mm256_dpbsud_epi32(sumi1, _mm256_set1_epi32(*((const int32_t *)(y +  8))), qx[2]);
    sumi2 = ggml_mm256_dpbsud_epi32(sumi2, _mm256_set1_epi32(*((const int32_t *)(y + 12))), qx[3]);
    sumi1 = ggml_mm256_dpbsud_epi32(sumi1, _mm256_set1_epi32(*((const int32_t *)(y + 16))), qx[4]);
    sumi2 = ggml_mm256_dpbsud_epi32(sumi2, _mm256_set1_epi32(*((const int32_t *)(y + 20))), qx[5]);
    sumi1 = ggml_mm256_dpbsud_epi32(sumi1, _mm256_set1_epi32(*((const int32_t *)(y + 24))), qx[6]);
    sumi2 = ggml_mm256_dpbsud_epi32(sumi2, _mm256_set1_epi32(*((const int32_t *)(y + 28))), qx[7]);
    return _mm256_add_epi32(sumi1, sumi2);
#else
    auto sumi1 = _mm256_setzero_si256();
    auto sumi2 = _mm256_setzero_si256();
    sumi1 = ggml_mm256_dpbusd_epi32(sumi1, qx[0], _mm256_set1_epi32(*((const int32_t *)(y +  0))));
    sumi2 = ggml_mm256_dpbusd_epi32(sumi2, qx[1], _mm256_set1_epi32(*((const int32_t *)(y +  4))));
    sumi1 = ggml_mm256_dpbusd_epi32(sumi1, qx[2], _mm256_set1_epi32(*((const int32_t *)(y +  8))));
    sumi2 = ggml_mm256_dpbusd_epi32(sumi2, qx[3], _mm256_set1_epi32(*((const int32_t *)(y + 12))));
    sumi1 = ggml_mm256_dpbusd_epi32(sumi1, qx[4], _mm256_set1_epi32(*((const int32_t *)(y + 16))));
    sumi2 = ggml_mm256_dpbusd_epi32(sumi2, qx[5], _mm256_set1_epi32(*((const int32_t *)(y + 20))));
    sumi1 = ggml_mm256_dpbusd_epi32(sumi1, qx[6], _mm256_set1_epi32(*((const int32_t *)(y + 24))));
    sumi2 = ggml_mm256_dpbusd_epi32(sumi2, qx[7], _mm256_set1_epi32(*((const int32_t *)(y + 28))));
    return _mm256_add_epi32(sumi1, sumi2);
#endif
}
...
static void mul_mat_q8_1_r8_q8_2(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
    ...
    if constexpr (nrc_y == 1) {
        ...
        auto dot = [&qx] (const int8_t * qy) {
            auto sumi = _mm256_setzero_si256();
#if defined(HAVE_VNNIINT8)
            sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_set1_epi32(*((const int32_t *)(qy +  0))), qx[0]);
            sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_set1_epi32(*((const int32_t *)(qy +  4))), qx[1]);
            sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_set1_epi32(*((const int32_t *)(qy +  8))), qx[2]);
            sumi = ggml_mm256_dpbsud_epi32(sumi, _mm256_set1_epi32(*((const int32_t *)(qy + 12))), qx[3]);
#else
            sumi = ggml_mm256_dpbusd_epi32(sumi, qx[0], _mm256_set1_epi32(*((const int32_t *)(qy +  0))));
            sumi = ggml_mm256_dpbusd_epi32(sumi, qx[1], _mm256_set1_epi32(*((const int32_t *)(qy +  4))));
            sumi = ggml_mm256_dpbusd_epi32(sumi, qx[2], _mm256_set1_epi32(*((const int32_t *)(qy +  8))));
            sumi = ggml_mm256_dpbusd_epi32(sumi, qx[3], _mm256_set1_epi32(*((const int32_t *)(qy + 12))));
#endif
            return sumi;
        };
        for (int ix = 0; ix < nrc_x; ix += 8) {
            const block_q8_1_r8 * iq8 = (const block_q8_1_r8 *)((const char *)vx + (ix+0)*bx);
...
static void mul_mat_q8_1_r8_q8_2(int n, const void * vx, size_t bx, const DataInfo& info, int nrc_x) {
    ...
    auto dot = [&qx] (const int8_t * qy) {
        ...
        return sumi;
#else
        auto sumi1 = _mm256_add_epi16(_mm256_maddubs_epi16(qx[0], _mm256_set1_epi32(*((const int32_t *)(qy +  0)))),
                                      _mm256_maddubs_epi16(qx[1], _mm256_set1_epi32(*((const int32_t *)(qy +  4))));
        auto sumi2 = _mm256_add_epi16(_mm256_maddubs_epi16(qx[2], _mm256_set1_epi32(*((const int32_t *)(qy +  8)))),
                                      _mm256_maddubs_epi16(qx[3], _mm256_set1_epi32(*((const int32_t *)(qy + 12))));
