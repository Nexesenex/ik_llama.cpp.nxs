...
static void dequantize_mul_mat_vec_iq5_nl_cuda(const void * vx, const dfloat * y, float * dst, const int ncols, const int nrows, cudaStream_t stream) {
    GGML_ASSERT(ncols % (GGML_CUDA_DMMV_X*2) == 0);
    const int block_num_y = (nrows + GGML_CUDA_MMV_Y - 1) / GGML_CUDA_MMV_Y;
    const dim3 block_nums(block_num_y, 1, 1);
    const dim3 block_dims(WARP_SIZE, GGML_CUDA_MMV_Y, 1);
    dequantize_mul_mat_vec<GGML_TYPE_IQ5_NL>
        <<<block_nums, block_dims, 0, stream>>>(vx, y, dst, ncols, nrows);
}
...
void ggml_cuda_op_dequantize_mul_mat_vec(
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_IQ5_NL:
            dequantize_mul_mat_vec_iq5_nl_cuda(src0_dd_i, src1_dfloat, dst_dd_i, ne00, row_diff, stream);
            break;
        case GGML_TYPE_Q8_0:
