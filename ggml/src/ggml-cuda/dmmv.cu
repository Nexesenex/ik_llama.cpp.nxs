...
static constexpr __device__ dequantize_kernel_t get_dequantize_kernel(ggml_type type) {
    return type == GGML_TYPE_Q4_0 ? dequantize_q4_0 :
        ...
        type == GGML_TYPE_Q8_1 ? dequantize_q8_1 :
...
static void dequantize_mul_mat_vec_q8_1_cuda(const void * vx, const dfloat * y, float * dst, const int ncols, const int nrows, cudaStream_t stream) {
    GGML_ASSERT(ncols % (GGML_CUDA_DMMV_X*2) == 0);
    const int block_num_y = (nrows + GGML_CUDA_MMV_Y - 1) / GGML_CUDA_MMV_Y;
    const dim3 block_nums(block_num_y, 1, 1);
    const dim3 block_dims(WARP_SIZE, GGML_CUDA_MMV_Y, 1);
    dequantize_mul_mat_vec<GGML_TYPE_Q8_1>
        <<<block_nums, block_dims, 0, stream>>>(vx, y, dst, ncols, nrows);
}

static void dequantize_mul_mat_vec_iq4_nl_cuda(const void * vx, const dfloat * y, float * dst, const int ncols, const int nrows, cudaStream_t stream) {
    GGML_ASSERT(ncols % (GGML_CUDA_DMMV_X*2) == 0);
    const int block_num_y = (nrows + GGML_CUDA_MMV_Y - 1) / GGML_CUDA_MMV_Y;
    const dim3 block_nums(block_num_y, 1, 1);
    const dim3 block_dims(WARP_SIZE, GGML_CUDA_MMV_Y, 1);
    dequantize_mul_mat_vec<GGML_TYPE_IQ4_NL>
        <<<block_nums, block_dims, 0, stream>>>(vx, y, dst, ncols, nrows);
}
...
void ggml_cuda_op_dequantize_mul_mat_vec(
    ...
    bool src1_convert_f16 =
        ...
        src0->type == GGML_TYPE_Q8_1 || src0->type == GGML_TYPE_F16  ||
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_IQ4_NL:
            dequantize_mul_mat_vec_iq4_nl_cuda(src0_dd_i, src1_dfloat, dst_dd_i, ne00, row_diff, stream);
            break;
        case GGML_TYPE_Q8_0:
            dequantize_mul_mat_vec_q8_0_cuda(src0_dd_i, src1_dfloat, dst_dd_i, ne00, row_diff, stream);
            break;
        case GGML_TYPE_Q8_1:
            dequantize_mul_mat_vec_q8_1_cuda(src0_dd_i, src1_dfloat, dst_dd_i, ne00, row_diff, stream);
            break;
        case GGML_TYPE_Q2_K:
...
bool ggml_cuda_dmmv_type_supported(ggml_type src0_type) {
    return src0_type == GGML_TYPE_Q4_0 || src0_type == GGML_TYPE_Q4_1 ||
        ...
        src0_type == GGML_TYPE_Q8_0 || src0_type == GGML_TYPE_Q8_1 || src0_type == GGML_TYPE_Q2_K ||
