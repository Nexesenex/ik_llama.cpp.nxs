...
#else
        constexpr int blocks_per_tile_x_row_iq2_k = 2*WARP_SIZE*2/QI8_0;
        const int i_dm_iq2_k = i/(QI8_0/4);
        x_df[i*blocks_per_tile_x_row_iq2_k + i_dm_iq2_k + 2*kqsx+0] = d * (((bxi->scales[kqsx] >> 0) & 0xf) - 8);
        x_df[i*blocks_per_tile_x_row_iq2_k + i_dm_iq2_k + 2*kqsx+1] = d * (((bxi->scales[kqsx] >> 4) & 0xf) - 8);
#endif // INT8_MMA_AVAILABLE
    }
}
...
#else
        constexpr int blocks_per_tile_x_row_iq2_k_r4 = 2*WARP_SIZE*2/QI8_0;
        const int i_dm_iq2_k_r4 = i/(QI8_0/4);
        x_df[i*blocks_per_tile_x_row_iq2_k_r4 + i_dm_iq2_k_r4 + 2*kqsx+0] = dl1;
        x_df[i*blocks_per_tile_x_row_iq2_k_r4 + i_dm_iq2_k_r4 + 2*kqsx+1] = dl2;
#endif // INT8_MMA_AVAILABLE
    }
}
