...
static constexpr __host__ __device__ tile_x_sizes mmq_get_dp4a_tile_x_sizes(ggml_type type, int mmq_y) {
    switch (type) {
        ...
        case GGML_TYPE_IQ5_NL  : return MMQ_DP4A_TXS_Q8_0;
...
static constexpr __host__ __device__ int mmq_get_mma_tile_x_k(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_IQ5_NL  : return MMQ_MMA_TILE_X_K_Q8_0;
...
#pragma unroll
    ...
    constexpr int blocks_per_tile_x_row = WARP_SIZE / QI4_0;
...
#pragma unroll
    for (int k01 = 0; k01 < WARP_SIZE; k01 += QR4_0*VDR_Q4_0_Q8_1_MMQ) {
        ...
        const int k0_qr  = k0 / QR4_0;
        const int k0_qi  = k0 / (QR4_0 * QI4_0);
...
#pragma unroll
                for (int l = 0; l < VDR_Q4_0_Q8_1_MMQ; ++l) {
                    ...
                    u[2*l+1] = y_qs[j_offset + kyqs + (l + QI4_0)];
                }

                sum[sum_base + i0/WARP_SIZE] += vec_dot_q4_0_q8_1_impl<VDR_Q4_0_Q8_1_MMQ>
                    (&x_qs[i*(WARP_SIZE + 1) + k0_qr], u,
                     x_df[i*(WARP_SIZE/QI4_0) + i/QI4_0 + k0_qi], y_ds[j_offset + k01_qi]);
            }
        }
    }
}
...
#pragma unroll
    ...
    constexpr int blocks_per_tile_x_row = WARP_SIZE / QI4_1;
    const int kbxd = threadIdx.x % blocks_per_tile_x_row;
...
#else
        const int i_dm = i/QI4_1;
        x_dm[i*blocks_per_tile_x_row + i_dm + kbxd] = bxi->dm;
#endif // INT8_MMA_AVAILABLE
    }
}
...
#pragma unroll
    ...
    constexpr int blocks_per_tile_x_row = WARP_SIZE / QI5_0;
    const int kbxd = threadIdx.x % blocks_per_tile_x_row;
...
#pragma unroll
    ...
    constexpr int blocks_per_tile_x_row = WARP_SIZE / QI5_1;
    const int kbxd = threadIdx.x % blocks_per_tile_x_row;
...
#pragma unroll
    ...
    constexpr int blocks_per_tile_x_row = WARP_SIZE / QI6_0;
    const int kbxd = threadIdx.x % blocks_per_tile_x_row;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps * QI6_0) {
        ...
        x_df[i*MMQ_MMA_TILE_X_K_Q8_0       + kbxd] = bxi->d;
#else
        const int i_dm = i/QI6_0;
        x_df[i*blocks_per_tile_x_row + i_dm + kbxd] = bxi->d;
#endif // INT8_MMA_AVAILABLE
    }
}

template <int mmq_y, int nwarps, bool need_check> static __device__ __forceinline__ void load_tiles_q6_1(
    const char * __restrict__ x, int * __restrict__ x_tile, const int & kbx0, const int & i_max, const int & stride) {
...
#pragma unroll
    for (int k01 = 0; k01 < WARP_SIZE; k01 += VDR_Q8_0_Q8_1_MMQ) {
        const int k0 = k00 + k01;
        const int k0_mod = k0 % WARP_SIZE;
        const int k0_qi8_0 = k0 / QI8_0;
        const int k0_qi8_1_mod = (k0 / QI8_1) % (WARP_SIZE / QI8_1);
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += WARP_SIZE) {
                const int i = i0 + threadIdx.x;
                const int i_xqs = i * (2 * WARP_SIZE + 1);
                const int i_xdf = i * (2 * WARP_SIZE / QI8_0) + i / (QI8_0 / 2);

                sum[j0 / nwarps * mmq_y / WARP_SIZE + i0 / WARP_SIZE] += vec_dot_q8_0_q8_1_impl<float, VDR_Q8_0_Q8_1_MMQ>
                    (&x_qs[i_xqs + k0], &y_qs[j_offset + k0_mod],
                     x_df[i_xdf + k0_qi8_0], y_df[j_offset + k0_qi8_1_mod]);
            }
        }
    }
}
...
static __device__ __forceinline__ void vec_dot_q8_0_q8_1_mma(
    ...
    const int i0_offset = i0 * MMQ_MMA_TILE_X_K_Q8_0;
    ...
    for (int k01 = 0; k01 < WARP_SIZE; k01 += QI8_0) {
        ...
        for (int l = 0; l < mma_C::ne/2; ++l) {
            ...
            if constexpr (ds_layout == MMQ_Q8_1_DS_LAYOUT_D4) {
                dB[l] = y_df[j_offset + k01_qi8_1];
            } else {
                dB[l] = __low2float(y_ds[j_offset + k01_qi8_1]);
            }
        }
        #pragma unroll
        for (int n = 0; n < ntx; ++n) {
            const int i_base = (i0 + n*mma_A::I) * MMQ_MMA_TILE_X_K_Q8_0;
            A[n].load(x_qs + i_base + k0, MMQ_MMA_TILE_X_K_Q8_0);
            #pragma unroll
            for (int l = 0; l < mma_C::ne/2; ++l) {
                const int i = i0 + n*mma_A::I + mma_C::get_i(2*l);
                dA[n][l] = x_df[i * MMQ_MMA_TILE_X_K_Q8_0 + k0_qi8_0];
            }
            mma_C C;
            C.mma_K8(A[n], B);
            #pragma unroll
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += WARP_SIZE) {
                ...
                const int i_dm_offset = i*(WARP_SIZE/QI5_1) + i/QI5_1;
...
#pragma unroll
    for (int k01 = 0; k01 < WARP_SIZE; k01 += QI8_0) {
        const int k0 = k00 + k01;
        const int k0_qi8_0_div2 = k0 / (QI8_0 / 2);
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += WARP_SIZE) {
                const int i = i0 + threadIdx.x;
                const int i_xqs = i * (2 * WARP_SIZE + 1);
                const int i_xdf = i * (2 * WARP_SIZE * 2 / QI8_0) + i / (QI8_0 / 4);

                sum[j0 / nwarps * mmq_y / WARP_SIZE + i0 / WARP_SIZE] += vec_dot_q8_0_16_q8_1_impl<QI8_0>(
                    &x_qs[i_xqs + k0],
                    &y_qs[j_offset + k01],
                    &x_df[i_xdf + k0_qi8_0_div2],
                    y_df[j_offset + k01 / QI8_1]);
            }
        }
    }
}
...
#pragma unroll
    for (int n = 0; n < ntx; ++n) {
        const int n_i_base = (i0 + n*mma_A::I) * MMQ_MMA_TILE_X_K_Q3_K;
#pragma unroll
        for (int k01 = 0; k01 < WARP_SIZE; k01 += 8) {
            const int k0 = k00 + k01;

            ((mma_A_K8 *) A[n])[k01/8].load(x_qs + n_i_base + k0, MMQ_MMA_TILE_X_K_Q3_K);
        }

#pragma unroll
        for (int l = 0; l < mma_C::ne/2; ++l) {
            const int i = i0 + n*mma_C::I + mma_C::get_i(2*l);
            const int i_offset = i * MMQ_MMA_TILE_X_K_Q3_K;

#pragma unroll
            for (int k01 = 0; k01 < WARP_SIZE; k01 += 4) {
                const int k0 = k00 + k01;

                dA[n][l][k01/4] = x_df[i_offset + k0/4];
            }
        }
    }
...
#pragma unroll
        for (int k01 = 0; k01 < WARP_SIZE; k01 += QR3_K*VDR_Q3_K_Q8_1_MMQ) {
            ...
            B[0].load(y_qs + j0_offset + (k01 + 0),        MMQ_TILE_Y_K);
            B[1].load(y_qs + j0_offset + (k01 + mma_B::K), MMQ_TILE_Y_K);
...
#pragma unroll
        for (int j0 = 0; j0 < mmq_x; j0 += nwarps) {
            const int j = j0 + threadIdx.y;
            const int j_offset = j*MMQ_TILE_Y_K;
            const int y_df_offset = j_offset + k01/QI8_1;

#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += WARP_SIZE) {
                const int i = i0 + threadIdx.x;
                const int i_qs_offset = i*(2*WARP_SIZE + 1);
                const int i_sc_offset = i*(WARP_SIZE/8) + i/8;
                const int sum_i = j0/nwarps*mmq_y/WARP_SIZE + i0/WARP_SIZE;

                const int8_t * scales = ((const int8_t *) (x_sc + i_sc_offset)) + k0/4;

                sum[sum_i] += vec_dot_q3_K_q8_1_impl_mmq(
                    &x_qs[i_qs_offset + k0], &y_qs[j_offset + k01], scales,
                    x_df[i], y_df[y_df_offset]);
            }
        }
    }
}
...
#pragma unroll
        for (int j0 = 0; j0 < mmq_x; j0 += nwarps) {
            const int j = j0 + threadIdx.y;
            const int j_offset = j*MMQ_TILE_Y_K;
            const int y_ds_offset = j_offset + k01/QI8_1;

#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += WARP_SIZE) {
                const int i = i0 + threadIdx.x;
                const int i_qs_offset = i*(WARP_SIZE + 1);
                const int i_sc_offset = i * (WARP_SIZE/8) + i/8 + k0/32;
                const int sum_i = j0/nwarps*mmq_y/WARP_SIZE + i0/WARP_SIZE;

                const uint8_t * sc = (const uint8_t *) &x_sc[i_sc_offset] + 2*(k01/16);

                sum[sum_i] += vec_dot_q4_K_q8_1_impl_mmq(
                    &x_qs[i_qs_offset + k0/2], &y_qs[j_offset + k01], sc, sc+8,
                    x_dm[i], &y_ds[y_ds_offset]);
            }
        }
    }
}
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps*8) {
        ...
        constexpr int blocks_per_tile_x_sc_row_q5_k = WARP_SIZE/8;
        const int i_sc_q5_k = i/8;
        x_sc[i*blocks_per_tile_x_sc_row_q5_k + i_sc_q5_k + ksc] = scales8;
    }
#endif // INT8_MMA_AVAILABLE
}
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += WARP_SIZE) {
                const int i = i0 + threadIdx.x;
                const int i_qs_offset = i*(QR5_K*WARP_SIZE + 1);
                const int i_sc_offset = i * (WARP_SIZE/8) + i/8 + k00/32;
                const int sum_i = j0/nwarps*mmq_y/WARP_SIZE + i0/WARP_SIZE;

                const uint8_t * sc = ((const uint8_t *) &x_sc[i_sc_offset]) + 2*(k01/16);

                sum[sum_i] += vec_dot_q5_K_q8_1_impl_mmq(
                    &x_qs[i_qs_offset + k0], &y_qs[j_offset + k01], sc, sc+8,
                    x_dm[i], &y_ds[y_ds_offset]);
            }
        }
    }
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += WARP_SIZE) {
                const int i = i0 + threadIdx.x;
                const int i_qs_offset = i*(QR6_K*WARP_SIZE + 1);
                const int i_sc_offset = i * (WARP_SIZE/8) + i/8 + k0/16;
                const int i_df_offset = i*(WARP_SIZE/QI6_K) + i/QI6_K;
                const int sum_i = j0/nwarps*mmq_y/WARP_SIZE + i0/WARP_SIZE;

                const int8_t * sc = ((const int8_t *) &x_sc[i_sc_offset]);

                sum[sum_i] += vec_dot_q6_K_q8_1_impl_mmq(
                    &x_qs[i_qs_offset + k0], &y_qs[j_offset + k01], sc,
                    x_df[i_df_offset], &y_df[y_df_offset]);
            }
        }
    }
}
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += 4*nwarps) {
        ...
        constexpr int blocks_per_tile_x_row_iq4_nl = WARP_SIZE/4;
        const int i_dm_iq4_nl = i/4;
        const block_iq4_nl * bxi = (const block_iq4_nl *)(x + i*stride) + kbx0 + kqsx;

#pragma unroll
        for (int j = 0; j < 4; ++j) {
            const int aux_q4 = get_int_b2(bxi->qs, j);
...
#ifdef INT8_MMA_AVAILABLE
        x_df[i*MMQ_MMA_TILE_X_K_Q8_0 + kqsx] = __half2float(bxi->d);
#else
        x_df[i*blocks_per_tile_x_row_iq4_nl + i_dm_iq4_nl + kqsx] = __half2float(bxi->d);
#endif // INT8_MMA_AVAILABLE
    }
}

template <int mmq_y, int nwarps, bool need_check> static __device__ __forceinline__ void load_tiles_iq5_nl(
    const char * __restrict__ x, int * __restrict__ x_tile, const int & kbx0, const int & i_max, const int & stride) {
    ...
    float * x_df = (float *) (x_qs + WARP_SIZE*2);
#else
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_IQ5_NL, mmq_y);
    int   * x_qs = (int   *)  x_tile;
    float * x_df = (float *) (x_qs + txs.qs);
#endif // INT8_MMA_AVAILABLE
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += 4*nwarps) {
        ...
        int qs0 = (v_low.x & ~mask_even) | (v_high.x & mask_even);
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += 4*nwarps) {
        ...
        constexpr int blocks_per_tile_x_row_mxfp4 = WARP_SIZE/4;
        const int i_dm_mxfp4 = i/4;
        const block_mxfp4 * bxi = (const block_mxfp4 *)(x + i*stride) + kbx0 + kqsx;
        helper.u = bxi->e ? uint32_t(bxi->e) << 23u : 0x00400000;

#pragma unroll
        for (int j = 0; j < 4; ++j) {
            const int aux_q4 = get_int_b1(bxi->qs, j);
...
#ifdef INT8_MMA_AVAILABLE
        x_df[i*MMQ_MMA_TILE_X_K_Q8_0 + kqsx] = 0.5f * helper.f;
#else
        x_df[i*blocks_per_tile_x_row_mxfp4 + i_dm_mxfp4 + kqsx] = 0.5f * helper.f;
#endif // INT8_MMA_AVAILABLE
    }
}
...
#else
        constexpr int blocks_per_tile_x_row_iq2_xxs = WARP_SIZE/4;
        const int i_dm_iq2_xxs = i/4;
        x_df[i*blocks_per_tile_x_row_iq2_xxs + i_dm_iq2_xxs + kqsx] = (ls*d + d/2)/4;
#endif // INT8_MMA_AVAILABLE
    }
}
...
#pragma unroll
        ...
        x_df[i*MMQ_MMA_TILE_X_K_Q3_K               + 2*kqsx+1] = ((ls >>    4)*d + d/2)/4;
#else
        constexpr int blocks_per_tile_x_row_iq2_s = 2*WARP_SIZE*2/QI8_0;
        const int i_dm_iq2_s = i/(QI8_0/4);
        x_df[i*blocks_per_tile_x_row_iq2_s + i_dm_iq2_s + 2*kqsx+0] = ((ls &  0x0F)*d + d/2)/4;
        x_df[i*blocks_per_tile_x_row_iq2_s + i_dm_iq2_s + 2*kqsx+1] = ((ls >>    4)*d + d/2)/4;
#endif // INT8_MMA_AVAILABLE
    }
}
...
#else
        constexpr int blocks_per_tile_x_row_iq3_xxs = WARP_SIZE/4;
        const int i_dm_iq3_xxs = i/4;
        x_df[i*blocks_per_tile_x_row_iq3_xxs + i_dm_iq3_xxs + kqsx] = (ls*d + d/2)/2;
#endif // INT8_MMA_AVAILABLE
    }
}
...
#else
        constexpr int blocks_per_tile_x_row_iq3_s = WARP_SIZE/4;
        const int i_dm_iq3_s = i/4;
        x_df[i*blocks_per_tile_x_row_iq3_s + i_dm_iq3_s + kqsx] = ls*d;
#endif // INT8_MMA_AVAILABLE
    }
}
...
#else
        constexpr int blocks_per_tile_x_row_iq1_s = WARP_SIZE/4;
        const int i_dm_iq1_s = i/4;
        x_ds[i*blocks_per_tile_x_row_iq1_s + i_dm_iq1_s + kqsx] = make_half2(d1q, d1q*delta);
#endif // INT8_MMA_AVAILABLE
    }
}
...
#else
        constexpr int blocks_per_tile_x_row_iq4_xs = WARP_SIZE/4;
...
template <int mmq_x, int mmq_y, int nwarps, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, nwarps, need_check, GGML_TYPE_Q6_1> {
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_q6_1<mmq_y, nwarps, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_1_q8_1_mma<mmq_x, mmq_y, nwarps>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_1_q8_1_dp4a<mmq_x, mmq_y, nwarps>;
};
...
template <int mmq_x, int mmq_y, int nwarps, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, nwarps, need_check, GGML_TYPE_IQ5_NL> {
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_iq5_nl<mmq_y, nwarps, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_q8_1_mma<mmq_x, mmq_y, nwarps, MMQ_Q8_1_DS_LAYOUT_D4>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_q8_1_dp4a<mmq_x, mmq_y, nwarps>;
};
...
static __global__ void mul_mat_q(
    const char * __restrict__ x, const char * __restrict__ yc, float * __restrict__ dst, float * __restrict__ tmp_fixup,
    const int ne00, const int ne01, const int stride01, const int ne10, const int ne11, const int stride11, const int ne0,
    const uint3 blocks_per_ne00_fd, const uint3 nty_fd) {
...
#if (defined(GGML_USE_HIPBLAS) && defined(__HIP_PLATFORM_AMD__)) || __CUDA_ARCH__ < CC_VOLTA
    {
        ...
        mul_mat_q_process_tile<type, mmq_x, nwarps, need_check, fixup>
            (x, yc, dst, tmp_fixup, ne00, ne01, stride01, ne10, ne11, stride11, ne0,
                blockIdx.x, blockIdx.y, 0, blocks_per_ne00_fd.z);
        return;
    }
    ...
    const int nty = nty_fd.z;                   // Number of tiles y

    // kbc == k block continuous, current index in continuous ijk space.
    int kbc      = int64_t(blockIdx.x)     *blocks_per_ne00_fd.z*ntx*nty / gridDim.x;
    int kbc_stop = int64_t(blockIdx.x + 1)*blocks_per_ne00_fd.z*ntx*nty / gridDim.x;

    kbc      -= fastmodulo((uint32_t)kbc,      blocks_per_ne00_fd) % blocks_per_iter;
    kbc_stop -= fastmodulo((uint32_t)kbc_stop, blocks_per_ne00_fd) % blocks_per_iter;

    // kb0 == k index when doing the matrix multiplication for an output tile.
    int kb0_start = fastmodulo((uint32_t)kbc, blocks_per_ne00_fd);
    int kb0_stop  = min(blocks_per_ne00_fd.z, (uint32_t)(kb0_start + kbc_stop - kbc));
    while (kbc < kbc_stop && kb0_stop == (int)blocks_per_ne00_fd.z) {
        int idx = fastdiv((uint32_t)kbc, blocks_per_ne00_fd);
        uint2 divmod = fast_div_modulo((uint32_t)idx, nty_fd);
        const int jt = divmod.x; // j index of current tile.
        const int it = divmod.y; // i index of current tile.
        ...
        kbc += blocks_per_ne00_fd.z;
        kbc -= fastmodulo((uint32_t)kbc, blocks_per_ne00_fd);

        kb0_start = 0;
        kb0_stop  = min(blocks_per_ne00_fd.z, (uint32_t)(kbc_stop - kbc));
    }
    ...
    int idx = fastdiv((uint32_t)kbc, blocks_per_ne00_fd);
    uint2 divmod = fast_div_modulo((uint32_t)idx, nty_fd);
    const int jt = divmod.x;
    const int it = divmod.y;
...
template <ggml_type type, int mmq_x, int nwarps, bool need_check>
__launch_bounds__(WARP_SIZE*nwarps/2, 1)
static __global__ void mul_mat_q_stream_k_fixup(
    float * __restrict__ dst, const float * __restrict__ tmp_last_tile, const int ne01, const int ne11, const int ne0,
    const uint3 blocks_per_ne00_fd, const uint3 nty_fd, const int nblocks_stream_k) {
    ...
    const int nty = nty_fd.z;

    const int i = blockIdx.y*WARP_SIZE + threadIdx.x;

    float sum[mmq_x / (nwarps/2)] = {0.0f};

    const int bidx0 = blockIdx.x;

    int kbc0      = int64_t(blockIdx.x)    *blocks_per_ne00_fd.z*ntx*nty / gridDim.x;
    int kbc0_stop = int64_t(blockIdx.x + 1)*blocks_per_ne00_fd.z*ntx*nty / gridDim.x;

    kbc0      -= fastmodulo((uint32_t)kbc0,      blocks_per_ne00_fd) % blocks_per_iter;
    kbc0_stop -= fastmodulo((uint32_t)kbc0_stop, blocks_per_ne00_fd) % blocks_per_iter;

    const bool did_not_have_any_data   = kbc0 == kbc0_stop;
    const bool wrote_beginning_of_tile = fastmodulo((uint32_t)kbc0, blocks_per_ne00_fd) == 0;
    const bool did_not_write_last      = fastdiv((uint32_t)kbc0, blocks_per_ne00_fd) == fastdiv((uint32_t)kbc0_stop, blocks_per_ne00_fd) && fastmodulo((uint32_t)kbc0_stop, blocks_per_ne00_fd) != 0;
    ...
    int kbc_stop = kbc0;
    while(true) {
        int kbc = int64_t(bidx)*blocks_per_ne00_fd.z*ntx*nty / nblocks_stream_k;
        kbc -= fastmodulo((uint32_t)kbc, blocks_per_ne00_fd) % blocks_per_iter;
...
#pragma unroll
        for (int j0 = 0; j0 < mmq_x; j0 += (nwarps/2)) {
            const int j = j0 + threadIdx.y;

            sum[j0/(nwarps/2)] += tmp_last_tile[bidx*(mmq_x*mmq_y) + j*mmq_y + i];
        }

        if (fastmodulo((uint32_t)kbc, blocks_per_ne00_fd) == 0 || fastdiv((uint32_t)kbc, blocks_per_ne00_fd) < fastdiv((uint32_t)kbc0, blocks_per_ne00_fd)) {
            break;
        }
        bidx--;
        kbc_stop = kbc;
    ...
    int idx = fastdiv((uint32_t)kbc0, blocks_per_ne00_fd);
    uint2 divmod = fast_div_modulo((uint32_t)idx, nty_fd);
    const int jt = divmod.x;
    const int it = divmod.y;
...
static void launch_mul_mat_q(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    const uint3 blocks_per_ne00_fd = init_fastdiv_values(args.ne00 / ggml_cuda_type_traits<type>::qk);
    ...
    // When tiling is efficient (>= 90%), use ntiles_dst blocks (one per tile) to skip the fixup
    const int ntiles_dst = ntx * nty;
    const int tiles_nwaves = (ntiles_dst + nsm - 1) / nsm;
    const int tiles_efficiency_percent = 100 * ntiles_dst / (nsm * tiles_nwaves);

    const dim3 block_nums_mmq = (GGML_CUDA_CC_IS_NVIDIA(cc) && tiles_efficiency_percent >= 90)
        ? dim3(ntiles_dst, 1, 1)
        : dim3(nsm, 1, 1);

    const bool fixup_needed = ntiles_dst % block_nums_mmq.x != 0;

    GGML_ASSERT(ntiles_dst * blocks_per_ne00_fd.z < (1u << 30));

    ggml_cuda_pool & pool = ctx.pool(id);
    ggml_cuda_pool_alloc<float> tmp_fixup(pool);
    if (fixup_needed) {
        tmp_fixup.alloc(block_nums_mmq.x * mmq_x*mmq_y);
    }
    ...
    if (args.ne01 % mmq_y == 0) {
        ...
        if (fixup_needed) {
            ...
            const dim3 block_dims_fixup(WARP_SIZE, MMQ_NWARPS/2, 1);
            const dim3 block_nums_fixup(block_nums_mmq.x, mmq_y / WARP_SIZE, 1);

            mul_mat_q_stream_k_fixup<type, mmq_x, MMQ_NWARPS, need_check><<<block_nums_fixup, block_dims_fixup, 0, stream>>>
                (args.dst, tmp_fixup.ptr, args.ne01, args.ne11, args.ne0, blocks_per_ne00_fd, nty_fd, block_nums_mmq.x);
        }
...
extern DECL_MMQ_CASE(GGML_TYPE_IQ5_NL);
