...
static constexpr __host__ __device__ tile_x_sizes mmq_get_dp4a_tile_x_sizes(ggml_type type, int mmq_y) {
    switch (type) {
        ...
        case GGML_TYPE_MXFP4_R8: return MMQ_DP4A_TXS_Q8_0;
...
static constexpr __host__ __device__ int mmq_get_mma_tile_x_k(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_MXFP4_R8: return MMQ_MMA_TILE_X_K_Q8_0;
...
template <int mmq_y, int nwarps, bool need_check> static __device__ __forceinline__ void load_tiles_mxfp4_r8(
    const char * __restrict__ x, int * __restrict__ x_tile, const int & kbx0, const int & i_max, const int & stride) {
    ...
    float * x_df = (float *) (x_qs + WARP_SIZE*2);
#else
    constexpr tile_x_sizes txs = mmq_get_dp4a_tile_x_sizes(GGML_TYPE_MXFP4_R8, mmq_y);
    int   * x_qs = (int   *)  x_tile;
    float * x_df = (float *) (x_qs + txs.qs);
#endif

    constexpr int qi = QI_MXFP4;
    const int kbx  = threadIdx.x / qi;
    const int kqsx = threadIdx.x % qi;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps) {
        ...
        int i8 = i / 8;
        int sr = i % 8;
        const block_mxfp4_r8 * bxi = (const block_mxfp4_r8 *)(x + i8*8*stride) + kbx0 + kbx;
        const int aux_q4 = *(const int *)(bxi->qs + 32*kqsx + 4*sr);
        const int2 v = get_int_from_table_16(aux_q4, kvalues_mxfp4);
        const int k0 = 8 * (threadIdx.x / 4) + threadIdx.x % 4;
#ifdef INT8_MMA_AVAILABLE
        x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + k0 + 0] = v.x;
    ...
    const int blocks_per_tile_x_row = WARP_SIZE / qi;
    const int kbxd = threadIdx.x % blocks_per_tile_x_row;

    union { float f; uint32_t u; } helper;

#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps * qi) {
        int i = i0 + threadIdx.y * qi + threadIdx.x / blocks_per_tile_x_row;
        if (need_check) { i = min(i, i_max); }
        int i8 = i / 8;
        int sr = i % 8;
        const block_mxfp4_r8 * bxi = (const block_mxfp4_r8 *)(x + i8*8*stride) + kbx0 + kbxd;
        helper.u = bxi->e[sr] >= 2 ? uint32_t(bxi->e[sr] - 1) << 23u : uval[bxi->e[sr]];
#ifdef INT8_MMA_AVAILABLE
        x_df[i*MMQ_MMA_TILE_X_K_Q8_0 + kbxd] = 0.5f * helper.f;
#else
        x_df[i*(WARP_SIZE/4) + i/4   + kbxd] = 0.5f * helper.f;
...
template <int mmq_x, int mmq_y, int nwarps, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, nwarps, need_check, GGML_TYPE_MXFP4_R8> {
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_mxfp4_r8<mmq_y, nwarps, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_q8_1_mma<mmq_x, mmq_y, nwarps, MMQ_Q8_1_DS_LAYOUT_D4>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_q8_1_dp4a<mmq_x, mmq_y, nwarps>;
};
...
extern DECL_MMQ_CASE(GGML_TYPE_MXFP4_R8);
