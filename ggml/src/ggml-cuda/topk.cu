...
// Compute top-K indices of each row of an F32 matrix.
// K is stored in dst->op_params[0] (0 = use ne[0] of dst for K).
...
void ggml_cuda_topk(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const int K = dst->op_params[0] > 0 ? dst->op_params[0] : (int)dst->ne[0];

    GGML_ASSERT(K <= 256 && "topk K must be <= 256");
    GGML_ASSERT(K <= (int64_t)dst->ne[0]);
    ...
    topk_f32<<<blocks_num, blocks_dim, shared_mem, stream>>>(src0_d, dst_d, ne00, K);
}
