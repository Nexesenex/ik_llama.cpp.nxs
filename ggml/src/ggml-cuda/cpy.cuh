...
static void ggml_cpy_f32_q_cuda(
    ...
    GGML_ASSERT(ne % qk == 0);
    const int num_blocks = (ne / qk + CUDA_CPY_BLOCK_SIZE - 1)/CUDA_CPY_BLOCK_SIZE;
    cpy_f32_q<cpy_blck, qk><<<num_blocks, CUDA_CPY_BLOCK_SIZE, 0, stream>>>
        (cx, cdst, ne, ne00, ne01, ne02, nb00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb13, cdst_indirect, graph_cpynode_index++);
}
...
static void ggml_cpy_q_f32_cuda(
    ...
    GGML_ASSERT(ne % qk == 0);
    const int num_blocks = (ne / qk + CUDA_CPY_BLOCK_SIZE - 1)/CUDA_CPY_BLOCK_SIZE;
    cpy_q_f32<cpy_blck_q_f32<dequant, qk>, qk><<<num_blocks, CUDA_CPY_BLOCK_SIZE, 0, stream>>>(
