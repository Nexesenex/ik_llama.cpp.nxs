...
#pragma unroll
                ...
                sum[sum_base + i0/warp_size] += vec_dot_q4_0_q8_1_impl<VDR_Q4_0_Q8_1_MMQ>
                    (&x_qs[i*(MMQ_TILE_NE_K + 1) + k0_qr], u,
                     x_df[i*(MMQ_TILE_NE_K/QI4_0) + i/QI4_0 + k0_qi], y_ds[j_offset + k01_qi]);
            }
        }
    }
}
...
#pragma unroll
                ...
                sum[sum_base + i0/warp_size] += vec_dot_q4_1_q8_1_impl<VDR_Q4_1_Q8_1_MMQ>
                    (&x_qs[i*(MMQ_TILE_NE_K + 1) + k0_qr], u,
                     x_dm[i*(MMQ_TILE_NE_K/QI4_1) + i/QI4_1 + k0_qi], y_ds[j_offset + k01_qi]);
            }
        }
    }
}
...
#else
        constexpr int blocks_per_tile_x_row_mxfp4 = MMQ_TILE_NE_K/QI_MXFP4;
        const int i_dm_mxfp4 = i/QI_MXFP4;
        x_df[i*blocks_per_tile_x_row_mxfp4 + i_dm_mxfp4 + kbxd] = ggml_cuda_e8m0_to_fp32(bxi->e)*0.5f;
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE)
    }
}
...
#pragma unroll
    for (int k01 = 0; k01 < MMQ_TILE_NE_K; k01 += VDR_Q8_0_Q8_1_MMQ) {
        const int k0 = k00 + k01;
        const int k0_mod = k0 % MMQ_TILE_NE_K;
        const int k0_qi8_0 = k0 / QI8_0;
        const int k0_qi8_1_mod = (k0 / QI8_1) % (MMQ_TILE_NE_K / QI8_1);
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += warp_size) {
                const int i = i0 + threadIdx.x;
                const int i_xqs = i * (2 * MMQ_TILE_NE_K + 1);
                const int i_xdf = i * (2 * MMQ_TILE_NE_K / QI8_0) + i / (QI8_0 / 2);
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += warp_size) {
                const int i = i0 + threadIdx.x;
                const int i_xqs = i * (2 * MMQ_TILE_NE_K + 1);
                const int i_xdf = i * (2 * MMQ_TILE_NE_K * 2 / QI8_0) + i / (QI8_0 / 4);
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += warp_size) {
                const int i = i0 + threadIdx.x;
                const int i_qs_offset = i*(2*MMQ_TILE_NE_K + 1);
                const int i_dm_offset = i*(MMQ_TILE_NE_K + 1);
                const int sum_i = j0/nwarps*mmq_y/warp_size + i0/warp_size;
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += warp_size) {
                const int i = i0 + threadIdx.x;
                const int i_qs_offset = i*(2*MMQ_TILE_NE_K + 1);
                const int i_sc_offset = i*(MMQ_TILE_NE_K/8) + i/8;
                const int sum_i = j0/nwarps*mmq_y/warp_size + i0/warp_size;
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += warp_size) {
                const int i = i0 + threadIdx.x;
                const int i_qs_offset = i*(MMQ_TILE_NE_K + 1);
                const int i_sc_offset = i * (MMQ_TILE_NE_K/8) + i/8 + k0/32;
                const int sum_i = j0/nwarps*mmq_y/warp_size + i0/warp_size;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps*rows_per_warp) {
        ...
        constexpr int blocks_per_tile_x_sc_row_q5_k = MMQ_TILE_NE_K/8;
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += warp_size) {
                const int i = i0 + threadIdx.x;
                const int i_qs_offset = i*(QR5_K*MMQ_TILE_NE_K + 1);
                const int i_sc_offset = i * (MMQ_TILE_NE_K/8) + i/8 + k00/32;
                const int sum_i = j0/nwarps*mmq_y/warp_size + i0/warp_size;
...
#pragma unroll
            for (int i0 = 0; i0 < mmq_y; i0 += warp_size) {
                const int i = i0 + threadIdx.x;
                const int i_qs_offset = i*(QR6_K*MMQ_TILE_NE_K + 1);
                const int i_sc_offset = i * (MMQ_TILE_NE_K/8) + i/8 + k0/16;
                const int i_df_offset = i*(MMQ_TILE_NE_K/QI6_K) + i/QI6_K;
                const int sum_i = j0/nwarps*mmq_y/warp_size + i0/warp_size;
...
#else
        constexpr int blocks_per_tile_x_row_iq4_nl = MMQ_TILE_NE_K/QI4_NL;
        const int i_dm_iq4_nl = i/QI4_NL;
        x_df[i*blocks_per_tile_x_row_iq4_nl + i_dm_iq4_nl + kbxd] = __half2float(bxi->d);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE)
    }
}

template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_iq5_nl(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += 4*nwarps) {
        ...
        x_df[i*MMQ_MMA_TILE_X_K_Q8_0             + kbxd] = __half2float(bxi->d);
#else
        x_df[i*(MMQ_TILE_NE_K/QI5_NL) + i/QI5_NL + kbxd] = __half2float(bxi->d);
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE)
    }
}
...
#else
        constexpr int blocks_per_tile_x_row_iq2_xxs = MMQ_TILE_NE_K/4;
...
#else
        constexpr int blocks_per_tile_x_row_iq2_xs = 2*MMQ_TILE_NE_K*2/QI8_0;
...
#pragma unroll
        ...
        x_df[i*MMQ_MMA_TILE_X_K_Q3_K                   + 2*kqsx+1] = ((ls >>    4)*d + d/2)/4;
#else
        constexpr int blocks_per_tile_x_row_iq2_s = 2*MMQ_TILE_NE_K*2/QI8_0;
...
#else
        constexpr int blocks_per_tile_x_row_iq3_xxs = MMQ_TILE_NE_K/4;
...
#else
        constexpr int blocks_per_tile_x_row_iq3_s = MMQ_TILE_NE_K/4;
...
#else
        constexpr int blocks_per_tile_x_row_iq1_s = MMQ_TILE_NE_K/4;
...
template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits_id<mmq_x, mmq_y, need_check, GGML_TYPE_Q6_1> {
...
template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits_id<mmq_x, mmq_y, need_check, GGML_TYPE_IQ5_NL> {
    //static constexpr int              vdr          = VDR_IQ5_NL_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_iq5_nl<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_q8_1_mma<mmq_x, mmq_y, MMQ_Q8_1_DS_LAYOUT_D4>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_q8_1_dp4a<mmq_x, mmq_y>;
};
