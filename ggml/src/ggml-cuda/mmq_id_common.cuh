...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_iq5_nl(
    ...
    constexpr int threads_per_row = MMQ_ITER_K / (4 * QR5_NL);
    constexpr int nrows = warp_size / threads_per_row;
    const int txi = warp_size > threads_per_row ? threadIdx.x % threads_per_row : threadIdx.x;
    const int kbx  = txi / QI5_NL;
    const int kqsx = txi % QI5_NL;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        uint32_t qh;
        memcpy(&qh, bxi->qh, sizeof(qh));

        const int aux_q5 = get_int_b2(bxi->qs, kqsx);
        const uint8_t qh_0 = (qh >> (2*kqsx)) & 0x03;
        const uint8_t qh_1 = (qh >> (2*kqsx + 16)) & 0x03;
        const int v0 = kvalues_iq5nl[(aux_q5 & 0x0F) | (qh_0 << 4)];
        const int v1 = kvalues_iq5nl[((aux_q5 >> 8) & 0x0F) | (qh_1 << 4)];
        const int k0 = kbx * (2 * QI5_NL) + kqsx;

#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE)
        x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + k0 + 0]      = v0;
        x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + k0 + QI5_NL] = v1;
#else
        x_qs[i*(2*MMQ_TILE_NE_K + 1) + k0 + 0]      = v0;
        x_qs[i*(2*MMQ_TILE_NE_K + 1) + k0 + QI5_NL] = v1;
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE)
    }
