...
// Fallback fixup kernel for when nblocks_stream_k < 4 * ntiles_dst (blocks_num.x not a multiple of ntiles_dst)
...
static void launch_fattn_new_mma(
    ...
    if (stream_k) {
        ...
        const int nblocks_stream_k_raw = std::min(max_blocks, ntiles_total);

        // Round down to a multiple of ntiles_total so that each output tile gets the same number of blocks.
        // Only do this if nblocks_stream_k_raw is at least 4x ntiles_total to avoid excessive loss of occupancy
        const int nblocks_stream_k = nblocks_stream_k_raw > 4 * ntiles_total
            ? (nblocks_stream_k_raw / ntiles_total) * ntiles_total
    ...
    if (stream_k) {
        if ((int)blocks_num.x % ntiles_total == 0 && (int)blocks_num.x > ntiles_total) {
            // Optimized fixup: nblocks_stream_k is a multiple of ntiles_total, launch one block per tile.
            const dim3 block_dim_combine(DV, 1, 1);
            const dim3 blocks_num_combine = {(unsigned)ntiles_total, ncols1, ncols2};

            flash_attn_stream_k_fixup<DV, ncols1, ncols2>
                <<<blocks_num_combine, block_dim_combine, 0, main_stream>>>
                ((float *) KQV->data, dst_tmp_meta.ptr, Q->ne[1], Q->ne[2], 1, K->ne[1], K->ne[2], 0, (int)blocks_num.x);
        } else if (ntiles_total % blocks_num.x != 0) {
            // Fallback fixup for the cases where nblocks_stream_k < ntiles_total.
            const dim3 block_dim_combine(DV, 1, 1);
