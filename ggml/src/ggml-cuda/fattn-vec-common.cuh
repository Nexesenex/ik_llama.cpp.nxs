...
static __device__ __forceinline__ int get_one_int_from_table_32_iq5_nl(const int & q4, const int & qh_bits) {
    const uint8_t * q0_8 = (const uint8_t *) &q4;
    const int idx0 = q0_8[0] | ((qh_bits >> 0) & 1) << 4;
    const int idx1 = q0_8[1] | ((qh_bits >> 1) & 1) << 4;
    const int idx2 = q0_8[2] | ((qh_bits >> 2) & 1) << 4;
    const int idx3 = q0_8[3] | ((qh_bits >> 3) & 1) << 4;
    const char4 val = make_char4(kvalues_iq5nl[idx0], kvalues_iq5nl[idx1], kvalues_iq5nl[idx2], kvalues_iq5nl[idx3]);
...
template<typename T, int Dk>
static __device__ __forceinline__ T vec_dot_fattn_vec_KQ_iq5_nl(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {

    const block_iq5_nl * K_iq5_nl = (const block_iq5_nl *) K_c;
    GGML_UNUSED(Q_v);
...
#pragma unroll
    for (int k_KQ_0 = 0; k_KQ_0 < Dk/sizeof(int); k_KQ_0 += WARP_SIZE) {
        ...
        if (std::is_same<T, half>::value) {
            const half2  * Q_ds = (const half2  *) Q_ds_v;
            sum += (T) (((half)sumi) * K_iq5_nl[ib].d * Q_ds[k_KQ_0/WARP_SIZE].x);
        } else
#endif // FP16_AVAILABLE
        {
            const float2 * Q_ds = (const float2 *) Q_ds_v;
            sum += (T) ((float)sumi * __half2float(K_iq5_nl[ib].d) * Q_ds[k_KQ_0/WARP_SIZE].x);
        }
    }
...
template<typename T, int Dk>
static __device__ __forceinline__ T vec_dot_fattn_vec_KQ_q6_1(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {
...
#pragma unroll
    for (int k_KQ_0 = 0; k_KQ_0 < Dk/sizeof(int); k_KQ_0 += WARP_SIZE) {
        ...
        const int ib    = k_KQ /  QI8_1;
        const int iqs4  = k_KQ %  QI6_1;  // 0...3
        const int shift = k_KQ & (QI8_1/2);

        const int vh = (get_int_b2(K_q6_1[ib].qh, iqs4%2) >> (4*(iqs4/2) + shift/2)) & 0x03030303;
        const int vl = (get_int_b2(K_q6_1[ib].qs, iqs4) >> shift) & 0x0F0F0F0F;
        const int v  = vl | (vh << 4);
        ...
        if (std::is_same<T, half>::value) {
            ...
            const half2 d5d8_m5s8 = K_q6_1[ib].dm * Q_ds[k_KQ_0/WARP_SIZE];
            const half2 sumid5d8_m5s8scaled = d5d8_m5s8 * make_half2(sumi, 1.0f/QI8_1);
            sum += (T) (__low2half(sumid5d8_m5s8scaled) + __high2half(sumid5d8_m5s8scaled));
        } else
#endif // FP16_AVAILABLE
        {
            ...
            const float sumid5d8   =  __low2float(K_q6_1[ib].dm)*Q_ds[k_KQ_0/WARP_SIZE].x * sumi;
            const float m5s8scaled = __high2float(K_q6_1[ib].dm)*Q_ds[k_KQ_0/WARP_SIZE].y / QI8_1;
...
#pragma unroll
    for (int k_KQ_0 = 0; k_KQ_0 < Dk/2; k_KQ_0 += WARP_SIZE) {
        ...
        const half2 K_ik = K_h2[k_KQ];
        const float2 Q_k = Q_f2[k_KQ_0/WARP_SIZE];
        ggml_cuda_mad(sum, __low2float(K_ik), Q_k.x);
        ggml_cuda_mad(sum, __high2float(K_ik), Q_k.y);
    }
...
template <typename T>
static __device__ __forceinline__ T dequantize_1_iq5_nl(const void * __restrict__ vx, const int64_t i) {
    const block_iq5_nl * x = (const block_iq5_nl *) vx;

    const int64_t ib    =  i          /  QK5_NL;
    const int     idq   =  i          %  QK5_NL;
    const int     iqs   =  i          % (QK5_NL/2);
    const int     shift = (i % QK5_NL) / (QK5_NL/2);
...
#ifdef FP16_AVAILABLE
    if (std::is_same<T, half>::value) {
        return x[ib].d * ((half) kvalues_iq5nl[idx]);
    }
#endif // FP16_AVAILABLE

    return (float)x[ib].d * ((float) kvalues_iq5nl[idx]);
}
...
template <typename T>
static __device__ __forceinline__ T dequantize_1_q6_1(const void * __restrict__ vx, const int64_t i) {
    const block_q6_1 * x = (const block_q6_1 *) vx;

    const int64_t ib    =  i  /  QK6_1;
    const int     idq   =  i  %  QK6_1;
    const int     iqs   =  i  % (QK6_1/2);
    const int     shift = idq / (QK6_1/2);

    const T   d  = __low2half(x[ib].dm);
    const T   m  = __high2half(x[ib].dm);
    const int ql = x[ib].qs[iqs] >> 4*shift;
    const int qh = x[ib].qh[idq%(QK6_1/4)] >> (4*((idq/(QK6_1/4))%2) + 2*shift);
    const int q  = ((ql & 0x0f) | ((qh & 0x03) << 4));

#ifdef FP16_AVAILABLE
    if (std::is_same<T, half>::value) {
        return ((half) d)*((half) q) + ((half) m);
    }
#endif // FP16_AVAILABLE

    return ((float) d)*((float) q) + ((float) m);
}
...
constexpr __device__ vec_dot_KQ_f16_t get_vec_dot_KQ_f16(ggml_type type_K) {
    return type_K == GGML_TYPE_Q4_0   ? vec_dot_fattn_vec_KQ_q4_0<half, Dk>   :
           ...
           type_K == GGML_TYPE_IQ5_NL ? vec_dot_fattn_vec_KQ_iq5_nl<half, Dk> :
           type_K == GGML_TYPE_Q5_0   ? vec_dot_fattn_vec_KQ_q5_0<half, Dk>   :
            type_K == GGML_TYPE_Q5_1   ? vec_dot_fattn_vec_KQ_q5_1<half, Dk>   :
            type_K == GGML_TYPE_Q6_1   ? vec_dot_fattn_vec_KQ_q6_1<half, Dk>   :
...
constexpr __device__ vec_dot_KQ_f32_t get_vec_dot_KQ_f32(ggml_type type_K) {
    return type_K == GGML_TYPE_Q4_0   ? vec_dot_fattn_vec_KQ_q4_0<float, Dk>   :
           ...
           type_K == GGML_TYPE_IQ5_NL ? vec_dot_fattn_vec_KQ_iq5_nl<float, Dk> :
           type_K == GGML_TYPE_Q5_0   ? vec_dot_fattn_vec_KQ_q5_0<float, Dk>   :
            type_K == GGML_TYPE_Q5_1   ? vec_dot_fattn_vec_KQ_q5_1<float, Dk>   :
            type_K == GGML_TYPE_Q6_1   ? vec_dot_fattn_vec_KQ_q6_1<float, Dk>   :
