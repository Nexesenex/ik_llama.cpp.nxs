...
static void ggml_sched_prepare_graph(ggml_backend_sched_t sched) {
    ...
    if (sched->split_mode_tensor_parallel) {
        ...
        for (int backend_id = 0; backend_id < sched->n_backends; ++backend_id) {
            ...
            size_t alloc_size = use_pipeline ? 2 * max_input_size : max_input_size;
            ...
            size_t half_size = max_input_size; // each slot's region
            int input_counter = 0;  // counts only splits with non-host inputs (for slot alternation)
            ...
            for (int i = 0; i < int(sched->backend_splits[backend_id].size()); ++i) {
                ...
                if (use_pipeline) {
                    // assign alternating slots for consecutive input splits
                    ...
                    // original reuse behavior: wrap around if would exceed half_size
                    // (half_size == max_input_size == alloc_size in non-pipeline mode)
