...
static void ggml_cuda_op_mul_mat_cublas(
    ...
    if (compute_capability >= CC_VOLTA && (src0->type == GGML_TYPE_F16 || src0->type == GGML_TYPE_BF16 || ggml_is_quantized(src0->type)) && ggml_is_contiguous(src0) && row_diff == src0->ne[1]) {
        ...
        const bool prec_f32 = dst->op_params[0] == GGML_PREC_F32;
...
static int ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst,
    ...
    } else if ((src0->type == GGML_TYPE_F16 || src0->type == GGML_TYPE_F32 || src0->type == GGML_TYPE_BF16) && (src1->type == src0->type || !any_gpus_with_slow_fp16)
