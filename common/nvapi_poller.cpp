...
// Hysteresis below the pause temperature at which a paused (too-hot) GPU
// resumes polling, avoiding flapping right at the limit. Only used as a
// fallback when no resume temp is configured via set_temp_limits.
static constexpr int HEAT_RESUME_HYSTERESIS_C = 10;
...
NvapiPoller::NvapiPoller(const std::vector<int>& devices, int interval_ms, int rounds)
    : devices(devices), intervals{interval_ms}, rounds(rounds) {
}
...
void NvapiPoller::get_penalties(std::vector<int> & out) const {
    std::lock_guard<std::mutex> lk(hot_mtx);
    out = penalties;
}
...
void NvapiPoller::thread_func() {
    ...
    // Resolve per-GPU intervals: a single value broadcasts to every card, more
    // values map positionally (WDDM slot order), missing values => 0 (off).
    std::vector<int> per_gpu(handles.size(), 0);
    if (intervals.size() == 1) {
        std::fill(per_gpu.begin(), per_gpu.end(), intervals[0]);
    } else {
        const size_t n = std::min(intervals.size(), handles.size());
        for (size_t k = 0; k < n; ++k) per_gpu[k] = intervals[k];
    }
    bool any_active = false;
    for (int v : per_gpu) if (v > 0) any_active = true;
    if (!any_active) {
        fprintf(stderr, "[NvapiPoller] no WDDM GPU has a positive interval, exiting\n");
        running = false;
        return;
    }

    fprintf(stderr, "[NvapiPoller] Started on %zu device(s), per-GPU interval(s) (ms):", handles.size());
    for (int v : per_gpu) fprintf(stderr, " %d", v);
    fprintf(stderr, "\n");
    ...
    if (!monitor_only.load()) {
        ...
        for (size_t k = 0; k < handles.size(); ++k) {
            const int itv = per_gpu[k];
            if (itv <= 0) continue; // GPU is off: no subscription
            ...
            util_settings[k].super.callbackPeriodms    = (NvU32) itv;
    ...
    // Seed from the member so penalties accumulated by a previous poller run
    // (e.g. an earlier TG phase) survive: they must never recover for the life
    // of the process, only grow.
    std::vector<int> local_penalties;
    {
        std::lock_guard<std::mutex> lk(hot_mtx);
        local_penalties = penalties;
    }
    if (local_penalties.size() < handles.size()) {
        local_penalties.resize(handles.size(), 0);
    }
    std::vector<int> hot_streak(handles.size(), 0); // consecutive hot cycles per card
    std::vector<std::chrono::steady_clock::time_point> next_due(handles.size(), {});
    ...
    while (should_run.load()) {
        ...
        for (size_t k = 0; k < handles.size(); ++k) {
            const int itv = per_gpu[k];
            if (itv <= 0) continue; // GPU interval off
            if (next_due[k] != std::chrono::steady_clock::time_point{} && now < next_due[k]) {
                continue; // not yet due
            }
            next_due[k] = now + std::chrono::milliseconds(itv);
            any_checked = true;

            // Per-card temperature protection. A hot card is skipped (no
            // queries, no burst) while the other GPUs keep being polled.
            const int pause_temp  = this->pause_temp.load();
            const int resume_temp = this->resume_temp.load() > 0 ? this->resume_temp.load()
                                                                 : pause_temp - HEAT_RESUME_HYSTERESIS_C;
            if (pause_temp > 0) {
                NV_GPU_THERMAL_SETTINGS thermal = {};
                thermal.version = NV_GPU_THERMAL_SETTINGS_VER;
                ...
                // Any sensor at/above the pause temp counts as a hot cycle for
                // this card this poll. Track consecutive hot cycles: two in a row
                // permanently lower the card's FMA scale by 16 (accumulates).
                bool card_hot = false;
                for (int i = 0; i < NVAPI_MAX_THERMAL_SENSORS_PER_GPU; ++i) {
                    if (thermal.sensor[i].controller == NVAPI_THERMAL_CONTROLLER_NONE) {
                        continue;
                    ...
                    } else if (t < resume_temp && local_hot[k]) {
                ...
                if (card_hot) {
                    hot_streak[k]++;
                    if (hot_streak[k] == 2) {
                        // Permanent FMA scale penalty: two consecutive hot cycles
                        // lower the multiplier by 16 (1/256ths), accumulates and
                        // never recovers for the life of the process. Only the
                        // first double-hot of a sustained episode counts; the
                        // streak keeps climbing, so a later double-hot (after the
                        // card cooled and the streak reset) lowers it again.
                        local_penalties[k] += 16;
                        fprintf(stderr, "[NvapiPoller] GPU %zu hot twice in a row: FMA scale permanently lowered by 16 (now -%d/256)\n",
                                k, local_penalties[k]);
                    }
                } else {
                    hot_streak[k] = 0; // cooled this cycle: reset the streak
                }
            }

            if (monitor_only.load() || local_hot[k]) {
                continue; // temperature tracking only, or too hot to burst
            }
