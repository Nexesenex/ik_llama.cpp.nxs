...
struct llama_init_result llama_init_from_gpt_params(gpt_params & params) {
    ...
    if (params.reranking) {
        ...
        if (llama_token_bos(model) == LLAMA_TOKEN_NULL) {
            LOG("%s: warning: model does not have a  BOS token, reranking will not work\n", __func__);
            ok = false;
        }

        if (llama_token_eos(model) == LLAMA_TOKEN_NULL) {
            LOG("%s: warning: model does not have an EOS token, reranking will not work\n", __func__);
            ok = false;
        }

        if (llama_token_sep(model) == LLAMA_TOKEN_NULL) {
            LOG("%s: warning: model does not have a  SEP token, reranking will not work\n", __func__);
            ok = false;
        }
    ...
    if (params.ctx_shift && !llama_kv_cache_can_shift(lctx)) {
        fprintf(stderr, "%s: KV cache shifting is not supported for this model (--no-context-shift to disable)'\n", __func__);
        llama_free_model(model);
        return iparams;
    }
    ...
    if (params.sparams.ignore_eos && llama_token_eos(model) == LLAMA_TOKEN_NULL) {
        LOG("%s: warning: model does not have an EOS token, ignoring --ignore-eos\n", __func__);
