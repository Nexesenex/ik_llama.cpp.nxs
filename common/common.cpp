...
bool gpt_params_find_arg(int argc, char ** argv, const std::string & arg, gpt_params & params, int & i, bool & invalid_param) {
    ...
    if (arg == "--n-cpu-moe-last" || arg == "-ncmoel") {
        ...
        params.ncmoe_last = n_layers;
        return true;
    }
    if (arg == "--n-cpu-moe-down-last" || arg == "-ncmoedl") {
        CHECK_ARG
        int32_t n_layers = std::stoi(argv[i]);
        if (n_layers < 0) {
            fprintf(stderr, "error: Invalid value for --n-cpu-moe-down-last: %d (must be >= 0)\n", n_layers);
            invalid_param = true;
            return true;
        }
        params.ncmoed_last = n_layers;
        return true;
    }
    if (arg == "--n-cpu-moe-up-gate-last" || arg == "-ncmoeugl") {
        CHECK_ARG
        int32_t n_layers = std::stoi(argv[i]);
        if (n_layers < 0) {
            fprintf(stderr, "error: Invalid value for --n-cpu-moe-up-gate-last: %d (must be >= 0)\n", n_layers);
            invalid_param = true;
            return true;
        }
        params.ncmoeug_last = n_layers;
        return true;
    }
    ...
    if (arg == "--n-cpu-dense" || arg == "-ncd") {
        ...
        int32_t n_layers = std::stoi(argv[i]);
        if (n_layers < 0) {
            fprintf(stderr, "error: Invalid value for --n-cpu-dense: %d (must be >= 0)\n", n_layers);
            invalid_param = true;
            return true;
        }
        params.ncd = n_layers;
    ...
    if (arg == "--n-cpu-dense-down" || arg == "-ncdd") {
        ...
        int32_t n_layers = std::stoi(argv[i]);
        if (n_layers < 0) {
            fprintf(stderr, "error: Invalid value for --n-cpu-dense-down: %d (must be >= 0)\n", n_layers);
            invalid_param = true;
            return true;
        }
        params.ncdd = n_layers;
        return true;
    }
    ...
    if (arg == "--n-cpu-dense-last" || arg == "-ncdl") {
        ...
        params.ncdl = n_layers;
        return true;
    }
    if (arg == "--n-cpu-dense-down-last" || arg == "-ncddl") {
        CHECK_ARG
        int32_t n_layers = std::stoi(argv[i]);
        if (n_layers < 0) {
            fprintf(stderr, "error: Invalid value for --n-cpu-dense-down-last: %d (must be >= 0)\n", n_layers);
            invalid_param = true;
            return true;
        }
        params.ncddl = n_layers;
        return true;
    }
    if (arg == "--n-cpu-dense-up-gate-last" || arg == "-ncdugl") {
        CHECK_ARG
        int32_t n_layers = std::stoi(argv[i]);
        if (n_layers < 0) {
            fprintf(stderr, "error: Invalid value for --n-cpu-dense-up-gate-last: %d (must be >= 0)\n", n_layers);
            invalid_param = true;
            return true;
        }
        params.ncdugl = n_layers;
        return true;
    }
...
void gpt_params_print_usage(int /*argc*/, char ** argv, const gpt_params & params) {
    ...
    options.push_back({ "*",           "       -cmoeug, --cpu-moe-up-gate", "keep all MoE up and gate weights in CPU memory (includes ffn_gate_up_exps when using -muge or for pre-merged models)"});

    options.push_back({ "*",           "       -ncmoed, --n-cpu-moe-down N", "keep MOE down weights of the first N layers in CPU memory"});
    options.push_back({ "*",           "       -ncmoeug, --n-cpu-moe-up-gate N", "keep MOE up and gate weights of the first N layers in CPU memory (includes ffn_gate_up_exps when using -muge or for pre-merged models)"});

    options.push_back({ "*",           "       -ncmoel, --n-cpu-moe-last N", "keep MoE weights of the last N layers in CPU memory"});
    options.push_back({ "*",           "       -ncmoedl, --n-cpu-moe-down-last N", "keep MoE down weights of the last N layers in CPU memory"});
    options.push_back({ "*",           "       -ncmoeugl, --n-cpu-moe-up-gate-last N", "keep MoE up and gate weights of the last N layers in CPU memory (includes ffn_gate_up_exps when using -muge or for pre-merged models)"});
    ...
    options.push_back({ "*",           "       -ncddl, --n-cpu-dense-down-last N", "keep down weights of the last N layers in CPU memory"});
    options.push_back({ "*",           "       -ncdugl, --n-cpu-dense-up-gate-last N", "keep up and gate weights of the last N layers in CPU memory"});
    ...
    if (llama_supports_gpu_offload()) {
        ...
        options.push_back({ "*",           "-sm,   --split-mode SPLIT_MODE",
                                                                        ...
                                                                        "  - tenpar: split model tensors and computation graph in parallel across GPUs\n"
...
struct llama_model_params common_model_params_to_llama(const gpt_params & params) {
    ...
    // Also support deprecated pinemb for backward compatibility
    if (!params.cuda_params.empty()) {
        // First check for pinmem
        ...
        // Also check for deprecated pinemb
        ...
        if (pos_pinemb != std::string::npos) {
            ...
            if (!pinemb_str.empty()) {
                try {
                    ...
                    // pinemb=0 -> pinmem=2, pinemb=1 -> pinmem=1
                    int pinmem_val = (pinemb_val == 0) ? 2 : 1;
                    ggml_backend_cuda_set_pinmem(pinmem_val);
                } catch (...) {
                    // Invalid value, keep default
                }
    ...
    mparams.ncmoe_last       = params.ncmoe_last;
    mparams.ncmoed_last      = params.ncmoed_last;
    mparams.ncmoeug_last     = params.ncmoeug_last;
    mparams.ncd              = params.ncd;
    mparams.ncdd             = params.ncdd;
    mparams.ncdug            = params.ncdug;
    mparams.ncdl             = params.ncdl;
    mparams.ncddl            = params.ncddl;
    mparams.ncdugl           = params.ncdugl;
    mparams.nclay            = params.nclay;
    mparams.nclayl           = params.nclayl;
