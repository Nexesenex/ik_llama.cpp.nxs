...
bool gpt_params_find_arg(int argc, char ** argv, const std::string & arg, gpt_params & params, int & i, bool & invalid_param) {
    ...
    if (arg == "-ctx-chk" || arg == "--ctx-checkpoints") {
        CHECK_ARG
        params.ctx_checkpoints_n = std::stoi(argv[i]);
        return true;
    }
    if (arg == "-ctx-chk-i" || arg == "--ctx-checkpoints-interval") {
        CHECK_ARG
        params.ctx_checkpoints_interval = std::stoi(argv[i]);
        return true;
    }
    if (arg == "-ctx-chk-ip" || arg == "--ctx-checkpoints-interval-progressive") {
        params.ctx_checkpoints_interval_progressive = true;
        return true;
    }
    if (arg == "-ctx-chk-t" || arg == "--ctx-checkpoints-tolerance") {
        CHECK_ARG
        params.ctx_checkpoints_tolerance = std::stoi(argv[i]);
        return true;
    }
    if (arg == "-ctx-chk-e" || arg == "--ctx-checkpoints-eviction") {
        CHECK_ARG
        params.ctx_checkpoint_eviction= common_checkpoint_eviction_from_name(std::string(argv[i]));
        return true;
    }
    if (arg == "-ctx-chk-ig" || arg == "--ctx-checkpoints-interval-gating") {
        params.ctx_checkpoints_interval_gating = true;
        return true;
    }
    if (arg == "-ctx-chk-mi" || arg == "--ctx-checkpoints-minimal-interval") {
        CHECK_ARG
...
void gpt_params_print_usage(int /*argc*/, char ** argv, const gpt_params & params) {
    ...
    options.push_back({ "*",           "-ctx-chk N, --ctx-checkpoints N",           "max number of context checkpoints to create per slot (default: %d)",params.ctx_checkpoints_n});
    options.push_back({ "*",           "-ctx-chk-i N, --ctx-checkpoints-interval N",  "minimum number of tokens between each context checkpoint.  (default: %d, <=0 disable)",params.ctx_checkpoints_interval});
    options.push_back({ "*",           "-ctx-chk-t N, --ctx-checkpoints-tolerance N", "the number of tokens before the full prompt to create the checkpoint.  (default: %d, <=0 disable)",params.ctx_checkpoints_tolerance});
    options.push_back({ "*",           "-ctx-chk-e NAME, --ctx-checkpoints-eviction NAME", "Eviction strategy for checkpoint. Accepts fifo, variance, streamlined and auto. Auto defaults to variance. Variance preserves coverage and maintains uniform interval. Streamlined evicts the checkpoint in the tightest cluster.  (default: variance)" });
    options.push_back({ "*",           "-ctx-chk-ig, --ctx-checkpoints-interval-gating", "Gate ALL checkpoint creation paths (PP-end, release, tolerance) through the interval check. Prevents checkpoint bursts from short follow-up tasks. (default: disabled)" });
    options.push_back({ "*",           "-ctx-chk-mi N, --ctx-checkpoints-minimal-interval N", "minimum pos_max gap between consecutive checkpoints. Skips creation if within N of the last one. (default: %d, 0 = disabled)", params.ctx_checkpoints_minimal_interval });
    options.push_back({ "*",           "-ctx-chk-ip, --ctx-checkpoints-interval-progressive",  "enable progressive checkpoint intervals. Requires --ctx-checkpoints N to be a multiple of eight. Intervals per eighth: 50%%, 75%%, 100%%, 125%%, 150%%, 175%%, 200%%, 225%%." });
    ...
    options.push_back({ "*",           "       --dry-multiplier N",     "DRY repetition penalty multiplier (default: %.1f, 0.0 = disabled)", (double)sparams.dry_multiplier });
    options.push_back({ "*",           "       --dry-base N",           "DRY repetition penalty base (default: %.2f)", (double)sparams.dry_base });
    options.push_back({ "*",           "       --dry-allowed-length N", "DRY allowed repeat length before penalty (default: %d)", sparams.dry_allowed_length });
    options.push_back({ "*",           "       --dry-penalty-last-n N", "DRY penalty scan window (default: %d, -1 = context size, 0 = disabled)", sparams.dry_penalty_last_n });
    ...
    options.push_back({ "*",           "       --r16-path",             "enable Q8_K_R16 path on VNNI256 (30% faster IQ4_XS)"});
